  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#202828] tracking-tight">System Notifications</h1>
          <p className="text-[#455250] mt-1">Global alerts, system updates, and messages.</p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 bg-white border border-[#E8E5DA] rounded-2xl">
          <div className="w-8 h-8 border-4 border-[#6fa3a0] border-t-transparent rounded-full animate-spin" />
        </div>
      ) : (
          ...
          <div className="divide-y divide-[#D3DFDA]">
            {notifications.map((notif) => (
              ...
              >
                ...
                {!notif.isRead && (
                  <span className="w-2.5 h-2.5 rounded-full bg-[#6fa3a0] shrink-0 self-center mt-1" />
                )}
              </button>
            ))}
          </div>
