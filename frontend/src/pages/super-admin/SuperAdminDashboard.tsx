...
const SuperAdminDashboard = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[#1E293B] tracking-tight">Super Admin Dashboard</h1>
        <p className="text-[#475569] mt-1">Platform overview and gym onboarding metrics.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6">
          <h2 className="text-xl font-bold text-[#1E293B] mb-6 border-b border-[#CCFBF1] pb-4">Onboarding Actions</h2>
          <div className="grid grid-cols-2 gap-4">
            <Link to="/super-admin/gyms/add" className="flex flex-col items-center justify-center p-6 bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl hover:border-[#0D9488] hover:bg-[#0D9488]/5 transition-all text-center">
              <Building2 size={32} className="text-[#0D9488] mb-3" />
              <span className="font-semibold text-[#1E293B]">Add Gym Manually</span>
            </Link>
            <Link to="/super-admin/customers" className="flex flex-col items-center justify-center p-6 bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl hover:border-[#0D9488] hover:bg-[#0D9488]/5 transition-all text-center">
              <Users size={32} className="text-[#0D9488] mb-3" />
              <span className="font-semibold text-[#1E293B]">View Customers</span>
            </Link>
            <Link to="/super-admin/gym-owners" className="flex flex-col items-center justify-center p-6 bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl hover:border-[#0D9488] hover:bg-[#0D9488]/5 transition-all text-center">
              <FileText size={32} className="text-[#0D9488] mb-3" />
              <span className="font-semibold text-[#1E293B]">Review Approvals</span>
            </Link>
            <Link to="/super-admin/invitations" className="flex flex-col items-center justify-center p-6 bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl hover:border-[#0D9488] hover:bg-[#0D9488]/5 transition-all text-center">
              <Send size={32} className="text-[#0D9488] mb-3" />
              <span className="font-semibold text-[#1E293B]">View Invitations</span>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6 border-b border-[#CCFBF1] pb-4">
            ...
            <div className="flex space-x-2 bg-[#FFFFFF] p-1 rounded-lg border border-[#CCFBF1]">
              <button 
                onClick={() => setRegistrationTab('owners')}
                className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${registrationTab === 'owners' ? 'bg-[#16A34A] text-white' : 'text-[#475569] hover:text-[#1E293B]'}`}
              >
                Gym Owners
              </button>
              <button 
                onClick={() => setRegistrationTab('customers')}
                className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${registrationTab === 'customers' ? 'bg-blue-500 text-white' : 'text-[#475569] hover:text-[#1E293B]'}`}
              >
                Customers
              </button>
            </div>
          ...
          {activities[registrationTab].length > 0 ? (
            <div className="flex-1 space-y-4 custom-scrollbar overflow-y-auto pr-2">
              {activities[registrationTab].map((user: any, idx: number) => (
                <div key={idx} className="flex items-center space-x-4 p-4 bg-[#FFFFFF] rounded-xl border border-[#CCFBF1] hover:border-[#16A34A]/30 transition-colors">
                  <div className={`w-2.5 h-2.5 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)] ${user.role === 'GYM_OWNER' ? 'bg-[#16A34A] shadow-[#16A34A]/50' : 'bg-blue-500 shadow-blue-500/50'}`} />
                  ...
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-md ${user.approvalStatus === 'APPROVED' ? 'bg-green-500/10 text-green-500' : user.approvalStatus === 'PENDING' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-[#E2E8F0] text-[#475569]'}`}>
                    {user.approvalStatus}
                  </span>
                </div>
              ))}
            ...
            <div className="flex-1 space-y-4 text-center flex flex-col items-center justify-center text-[#475569]">
