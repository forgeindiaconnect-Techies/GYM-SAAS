      ...
      <div className="bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl overflow-hidden">
        <table className="w-full text-left text-sm text-[#455250]">
          ...
          <tbody className="divide-y divide-[#D3DFDA]">
            {invites.map(invite => (
              <tr key={invite.id} className="hover:bg-[#F1F5F3]">
                <td className="px-6 py-4 font-semibold text-[#202828]">{invite.gymName}</td>
                <td className="px-6 py-4">
                  <div className="text-[#202828]">{invite.owner}</div>
                ...
                <td className="px-6 py-4 flex space-x-3">
                  ...
                  <button onClick={() => handleCancel(invite)} className="text-[#6fa3a0] hover:text-teal-400 flex items-center gap-1" title="Cancel"><Trash2 size={16}/></button>
                </td>
              </tr>
            ))}
