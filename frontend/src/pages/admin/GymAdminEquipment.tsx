...
const GymAdminEquipment = () => {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'Out of Order': return 'bg-[#6fa3a0]/10 text-[#6fa3a0] border-[#6fa3a0]/20';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {isLoading ? (
          <div className="col-span-1 md:col-span-2 xl:col-span-3 text-center py-10 text-[#455250]">Loading equipment...</div>
        ) : equipmentToDisplay.length === 0 ? (
          <div className="col-span-1 md:col-span-2 xl:col-span-3 text-center py-16 bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl flex flex-col items-center justify-center">
            <div className="w-16 h-16 bg-[#F1F5F9] rounded-full flex items-center justify-center mb-4">
              <Wrench size={32} className="text-[#A8ADA9]" />
            </div>
            <h3 className="text-xl font-bold text-[#202828] mb-2">No Equipment Found</h3>
            <p className="text-[#455250] max-w-md">There is no equipment in this branch yet. Add your first piece of equipment to start managing your inventory.</p>
            <button onClick={() => setShowAddModal(true)} className="mt-6 flex items-center space-x-2 px-6 py-2 bg-[#164A4A] text-white rounded-xl font-bold hover:bg-[#C6A77D] transition-colors">
              <Plus size={18} />
              <span>Add Equipment</span>
            </button>
          </div>
          ...
          .map((eq: any, idx: number) => {
            ...
            return (
          <div key={eq._id || eq.id || idx} className="bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl p-6 hover:border-[#164A4A]/30 transition-all group relative overflow-hidden">
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="flex items-center space-x-2 mb-1">
                  <span className="text-xs font-bold text-[#455250] bg-[#FFFFFF] px-2 py-1 rounded-md">{eq.id || `EQ00${idx+1}`}</span>
                  <span className="text-xs font-semibold text-[#164A4A] uppercase tracking-wider">{eq.category || 'General'}</span>
                </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-t border-[#D3DFDA]">
                <span className="text-sm text-[#455250]">Current Status</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-[#455250]">Next Service</span>
                <span className="text-sm font-semibold text-[#202828] bg-[#FFFFFF] px-3 py-1 rounded-lg">
                  {eq.nextService || 'N/A'}
                </span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#FFFFFF] rounded-2xl max-w-2xl w-full shadow-2xl border border-[#D3DFDA] mt-10 mb-10">
            <div className="p-6 border-b border-[#D3DFDA] flex justify-between items-center sticky top-0 bg-white z-10 rounded-t-2xl">
              <h2 className="text-2xl font-bold text-[#202828]">Add New Equipment</h2>
              <button onClick={() => setShowAddModal(false)} className="text-[#455250] hover:text-[#202828]"><X size={24} /></button>
            </div>
            <form onSubmit={handleAddEquipment} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]" placeholder="e.g. Treadmill Series X" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#455250] mb-1">Category *</label>
                  <select required value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]">
                ...
                <div>
                  ...
                  <input value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]" placeholder="e.g. LifeFitness" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#455250] mb-1">Quantity *</label>
                  <input type="number" min="1" required value={formData.quantity} onChange={e => setFormData({...formData, quantity: parseInt(e.target.value) || 1})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#455250] mb-1">Condition</label>
                  <select value={formData.condition} onChange={e => setFormData({...formData, condition: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]">
                ...
                <div>
                  ...
                  <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]">
                ...
                <div>
                  ...
                  <input type="date" value={formData.nextService} onChange={e => setFormData({...formData, nextService: e.target.value})} className="w-full border border-[#D3DFDA] rounded-lg px-4 py-2 outline-none focus:border-[#164A4A]" />
                </div>
              </div>
      ...
      {selectedEq && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#FFFFFF] rounded-2xl max-w-md w-full shadow-2xl border border-[#D3DFDA] mt-20 mb-10 overflow-hidden">
            <div className="p-6 border-b border-[#D3DFDA] flex justify-between items-center bg-[#F2EFE8]">
              <h2 className="text-xl font-bold text-[#202828]">Equipment Details</h2>
              <button onClick={() => setSelectedEq(null)} className="text-[#455250] hover:text-[#202828]"><X size={24} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <span className="text-sm font-bold text-[#455250]">Name</span>
                <p className="text-[#202828] font-semibold">{selectedEq.name}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <span className="text-sm font-bold text-[#455250]">Category</span>
                  <p className="text-[#202828] font-semibold capitalize">{selectedEq.category}</p>
                </div>
                <div>
                  <span className="text-sm font-bold text-[#455250]">Brand</span>
                  <p className="text-[#202828] font-semibold">{selectedEq.brand || 'N/A'}</p>
                </div>
                <div>
                  <span className="text-sm font-bold text-[#455250]">Quantity</span>
                  <p className="text-[#202828] font-semibold">{selectedEq.quantity || 1}</p>
                </div>
                <div>
                  <span className="text-sm font-bold text-[#455250]">Status</span>
                  <p className="text-[#202828] font-semibold">{selectedEq.status || (selectedEq.condition === 'Poor' ? 'Maintenance' : 'Active')}</p>
                </div>
                <div>
                  <span className="text-sm font-bold text-[#455250]">Condition</span>
                  <p className="text-[#202828] font-semibold">{selectedEq.condition || 'N/A'}</p>
                </div>
                <div>
                  <span className="text-sm font-bold text-[#455250]">Next Service Date</span>
                  <p className="text-[#202828] font-semibold">{selectedEq.nextService || 'N/A'}</p>
                </div>
              </div>
            </div>
            <div className="p-6 border-t border-[#D3DFDA] bg-[#F2EFE8] flex justify-end">
              <button onClick={() => setSelectedEq(null)} className="px-6 py-2 bg-[#FFFFFF] border border-[#D3DFDA] text-[#202828] font-bold hover:bg-gray-100 rounded-xl transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
