...
const GymStoreSettings = () => {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-[#202828] tracking-tight">Store Settings</h1>
        <p className="text-[#455250] mt-1">Gym Store feature status and configuration.</p>
      </div>

      {status?.enabled ? (
        <>
          <div className="bg-white border border-[#D3DFDA] rounded-2xl p-6 flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-green-100 text-[#164A4A] flex items-center justify-center shrink-0">
              <Store size={24} />
            </div>
            <div>
              <h2 className="text-xl font-bold text-[#202828] flex items-center gap-2">
                Gym Store is Active <CheckCircle2 size={18} className="text-[#164A4A]" />
              </h2>
              <p className="text-[#455250] mt-1">
              ...
              <p className="text-[#455250] mt-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            {features.map((f) => (
              <div key={f} className="bg-white border border-[#D3DFDA] rounded-2xl p-4 flex items-start gap-3">
                <CheckCircle2 className="text-[#164A4A] shrink-0 mt-0.5" size={18} />
                <p className="text-sm text-[#202828] font-medium">{f}</p>
              </div>
            ))}
          </div>
        </>
        ...
        <div className="bg-white border border-[#D3DFDA] rounded-2xl p-6">
          <h2 className="text-xl font-bold text-[#202828] mb-2">Gym Store is not active for your gym</h2>
          <p className="text-[#455250] mb-4">
            Upgrade to <span className="font-bold text-[#164A4A]">Premium</span> to unlock the full store. Your current plan:{' '}
            <span className="font-semibold">{status?.userEntitlement?.plan || 'Basic'}</span> (
            {status?.userEntitlement?.status || 'Inactive'}).
          </p>
          <Link to="/admin/subscription" className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#164A4A] to-[#6fa3a0] text-white font-bold rounded-xl shadow-lg shadow-green-200 hover:opacity-90 transition-opacity">
      ...
      <div className="bg-white border border-[#D3DFDA] rounded-2xl p-6">
        <h2 className="text-lg font-bold text-[#202828] mb-3">How the store works</h2>
        <ol className="space-y-3 text-sm text-[#455250] list-decimal list-inside pl-1">
