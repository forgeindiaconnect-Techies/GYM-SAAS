...
const GymStoreDashboard = () => {
  ...
  if (!status?.enabled) {
    return (
      <div className="max-w-2xl mx-auto mt-16 bg-white border border-[#CCFBF1] rounded-3xl p-8 shadow-sm text-center">
        ...
        <h1 className="text-2xl font-bold text-[#1E293B] mb-3">Gym Store is a Premium feature</h1>
        <p className="text-[#475569] mb-2">
          Sell gym merchandise, supplements, and accessories directly to your members with the
          <span className="font-bold text-[#16A34A]"> Premium </span>
          plan.
        </p>
        <p className="text-[#475569] mb-8">
          Your current subscription:{' '}
          <span className="font-bold text-[#1E293B]">{status?.userEntitlement?.plan || status?.gymEntitlement?.plan || 'Basic'}</span>{' '}
          ({status?.userEntitlement?.status || status?.gymEntitlement?.status || 'Inactive'})
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link
            to="/admin/subscription"
            className="px-6 py-3 bg-gradient-to-r from-[#16A34A] to-[#0D9488] text-white font-bold rounded-xl shadow-lg shadow-green-200 hover:opacity-90 transition-opacity"
          >
            Upgrade to Premium
          </Link>
          <Link
            to="/admin/dashboard"
            className="px-6 py-3 bg-gray-100 text-[#475569] font-bold rounded-xl hover:bg-gray-200 transition-colors"
          >
            Back to Dashboard
          </Link>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#475569] mt-1">Sell products online and in-gym. Manage your store here.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-white border border-[#CCFBF1] rounded-2xl p-5 flex items-start justify-between">
            <div>
              <p className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider mb-1">{c.label}</p>
              <h3 className="text-2xl font-black text-[#1E293B]">{c.value}</h3>
              <p className="text-xs text-[#475569] mt-1">{c.sub}</p>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Recent orders */}
        <div className="lg:col-span-2 bg-white border border-[#CCFBF1] rounded-2xl overflow-hidden">
          <div className="p-6 border-b border-[#CCFBF1] flex items-center justify-between">
            <h3 className="text-lg font-bold text-[#1E293B]">Recent Orders</h3>
            <Link to="/admin/store/orders" className="text-sm font-bold text-[#16A34A] hover:underline flex items-center gap-1">
              View all <ChevronRight size={16} />
            </Link>
          </div>
          {recentOrders.length === 0 ? (
            <div className="p-10 text-center text-[#64748B]">No orders yet. Share your store with members to get started.</div>
          ) : (
            <div className="divide-y divide-[#CCFBF1]">
              {recentOrders.map((o: any) => (
                <div key={o._id} className="flex items-center justify-between px-6 py-4" onClick={() => navigate('/admin/store/orders')}>
                  <div>
                    <p className="font-bold text-[#1E293B] text-sm">{o.orderNumber}</p>
                    <p className="text-xs text-[#475569]">{(o.customerId?.firstName || 'Member')} {(o.customerId?.lastName || '')} · {new Date(o.createdAt).toLocaleDateString()}</p>
                  </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <Link
                key={q.label}
                to={q.path}
                className="flex items-center justify-between px-4 py-3 bg-[#F0FDFA] rounded-xl hover:bg-[#CCFBF1] transition-colors text-[#1E293B] font-semibold text-sm"
              >
                {q.label} <ChevronRight size={16} className="text-[#16A34A]" />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 bg-white border border-[#CCFBF1] rounded-2xl px-5 py-4 text-sm text-[#475569]">
        <Bell className="text-[#16A34A]" size={18} />
        <span>
