...
const GymStoreProducts = () => {
    ...
    if (!form.hasVariants) {
      ...
      for (const v of form.variants) {
        ...
        if (v.discountPrice !== '' && Number(v.discountPrice) > Number(v.price)) {
          alert('Discount price cannot be greater than price in variants.');
          return;
        }
      }
    }
  ...
  const inputCls = 'w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-lg px-3 py-2 text-xs text-[#202522] focus:border-[#34483F] outline-none';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#202522] tracking-tight">Products</h1>
          <p className="text-[#4A514D] mt-1">Add, edit and manage your gym store products.</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-3">
        ...
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-3 py-2 text-sm text-[#202522] focus:border-[#34483F] outline-none lg:w-48">
          <option value="all">All Categories</option>
          {categories.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-3 py-2 text-sm text-[#202522] focus:border-[#34483F] outline-none lg:w-40">
          <option value="all">All Status</option>
          <option value="Active">Active</option>
          <option value="Inactive">Inactive</option>
        </select>
        <select value={inventory} onChange={(e) => setInventory(e.target.value)} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-3 py-2 text-sm text-[#202522] focus:border-[#34483F] outline-none lg:w-48">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white border border-[#DCD9CD] rounded-2xl">
          <Package className="mx-auto text-[#34483F]/30 mb-4" size={56} />
          <p className="text-[#727975] font-medium">No products found.</p>
        </div>
      ) : (
          ...
          {products.map((p) => {
            ...
            return (
              <div key={p._id} className="bg-white border border-[#DCD9CD] rounded-2xl overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                <div className="h-40 bg-[#F5F3EE] relative">
                  {p.image ? (
                  ...
                  <span className={`absolute top-2 left-2 px-2.5 py-1 rounded-full text-xs font-bold ${
                    totalStock <= 0 ? 'bg-red-100 text-red-700' : isLow ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'
                  }`}>
                  ...
                  {!p.hasVariants && p.discountPrice != null && p.discountPrice < p.sellingPrice && (
                    <span className="absolute top-2 right-2 px-2.5 py-1 rounded-full text-xs font-bold bg-[#34483F] text-white">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <h3 className="font-bold text-[#202522] truncate">{p.name}</h3>
                  {p.brand && <p className="text-xs text-[#4A514D]">{p.brand}{p.sku && !p.hasVariants ? ` · ${p.sku}` : ''}</p>}
                  ...
                  <div className="mt-3 text-xs text-[#4A514D] grid grid-cols-2 gap-1">
                    <span>Channels: {p.availability}</span>
                    <span>Fulfil: {p.fulfilmentType}</span>
                  </div>
                  {p.hasVariants && (
                    <div className="mt-2 text-xs font-bold text-[#8FA89B]">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-4xl w-full p-6 shadow-2xl relative my-8">
            <button onClick={() => setModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors">
              <X size={20} />
            </button>
            <h2 className="text-xl font-bold text-[#202522] mb-5">{editing ? 'Edit Product' : 'Add Product'}</h2>

            <div className="space-y-6">
              {/* PRODUCT INFORMATION */}
              <div>
                <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider mb-3">Product Information</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Product Type *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Product Name *</label>
                    <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} placeholder="e.g., Whey Protein 1kg" />
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Category *</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Brand</label>
                    <input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} className={inputCls} placeholder="e.g., MuscleBlaze" />
                  </div>
                  {!form.hasVariants && (
                    <div>
                      <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">SKU</label>
                      <input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} className={inputCls} placeholder="Unique code (optional)" />
                    </div>
                  )}
                  <div className="sm:col-span-2">
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Description</label>
              ...
              {typeConfig.fields.length > 0 && (
                <div className="pt-4 border-t border-gray-100">
                  <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider mb-3">Product Details</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {typeConfig.fields.map(field => (
                      <div key={field.name}>
                        <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">{field.label}</label>
                        {field.type === 'select' ? (
              ...
              {typeConfig.variantFields.length > 0 && (
                <div className="pt-4 border-t border-gray-100">
                  <label className="flex items-center gap-2 cursor-pointer w-max">
                    ...
                    }} className="accent-[#34483F] w-4 h-4 rounded" />
                    <span className="text-sm font-bold text-[#202522]">This product has multiple options/variants</span>
                  </label>
                </div>
              )}
              ...
              {form.hasVariants ? (
                <div className="pt-4 border-t border-gray-100">
                  <div className="flex justify-between items-center mb-3">
                     <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider">Variants</h3>
                     <button onClick={addVariantRow} type="button" className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 text-xs font-bold flex items-center gap-1"><Plus size={14}/> Add Variant</button>
                  </div>
                  <div className="overflow-x-auto rounded-lg border border-[#DCD9CD]">
                    <table className="w-full text-left text-xs whitespace-nowrap">
                      <thead className="bg-[#F5F3EE] text-[#4A514D]">
                        <tr>
                      ...
                      <tbody>
                        {form.variants.length === 0 ? (
                          ...
                          form.variants.map((v, idx) => (
                             <tr key={idx} className="border-t border-[#DCD9CD]">
                                {typeConfig.variantFields.map(f => (
                                   <td key={f.name} className="p-2">
                                     <input value={v.attributes[f.name] || ''} onChange={(e) => updateVariant(idx, 'attributes', { ...v.attributes, [f.name]: e.target.value })} className="w-full border border-gray-200 rounded px-2 py-1.5 text-xs outline-none focus:border-[#34483F] min-w-[100px]" placeholder={f.placeholder} />
                                   </td>
                                ))}
                                ...
                                <td className="p-2"><input type="number" value={v.discountPrice} onChange={(e) => updateVariant(idx, 'discountPrice', e.target.value)} className="w-full border border-gray-200 rounded px-2 py-1.5 text-xs w-20" placeholder="0" /></td>
                                ...
                                <td className="p-2 text-center"><button type="button" onClick={() => removeVariant(idx)} className="text-red-500 hover:text-red-700 bg-red-50 p-1.5 rounded"><Trash2 size={14}/></button></td>
                             </tr>
                          ))
                        )}
                      </tbody>
                ...
                <div className="pt-4 border-t border-gray-100">
                  <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider mb-3">Pricing & Inventory</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Selling Price (₹) *</label>
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Discount Price (₹)</label>
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Stock Quantity *</label>
                      <input type="number" min={0} value={form.stock} onChange={(e) => setForm({ ...form, stock: Number(e.target.value) })} className={inputCls} />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Low Stock Alert At</label>
              ...
              {/* SALES SETTINGS */}
              <div className="pt-4 border-t border-gray-100">
                <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider mb-3">Sales Settings</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Status</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Sell Online</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[#727975] uppercase tracking-wider mb-1 block">Fulfilment</label>
              ...
              <div className="pt-4 border-t border-gray-100">
                <h3 className="text-xs font-bold text-[#34483F] uppercase tracking-wider mb-3">Product Image</h3>
                <div className="flex items-center gap-3">
                  <div className="w-16 h-16 rounded-xl border border-[#DCD9CD] overflow-hidden bg-[#F5F3EE] flex items-center justify-center shrink-0">
                    {previewUrl ? (
                      <img src={previewUrl} alt="preview" className="w-full h-full object-cover" />
                    ) : (
                      <ImageIcon className="text-[#34483F]/30" size={24} />
                    )}
                  </div>
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      {uploadingImg ? (
                        <Loader2 className="animate-spin text-[#34483F]" size={16} />
                      ) : (
                        <label className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#F5F3EE] border border-[#DCD9CD] rounded-lg text-xs font-bold text-[#34483F] cursor-pointer hover:bg-[#DCD9CD] transition-colors">
            ...
            <div className="mt-8 flex gap-3">
              <button onClick={() => setModalOpen(false)} className="flex-1 py-3 bg-gray-100 text-[#4A514D] text-sm font-bold rounded-xl hover:bg-gray-200 transition-colors">
                Cancel
              </button>
              <button onClick={saveProduct} disabled={saving} className="flex-1 py-3 bg-gradient-to-r from-[#34483F] to-[#8FA89B] text-white text-sm font-bold rounded-xl shadow-lg shadow-green-200 hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
