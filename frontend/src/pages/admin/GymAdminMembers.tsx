...
const GymAdminMembers = () => {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'Active': return 'bg-[#164A4A]/10 text-[#164A4A] border-[#164A4A]/20';
      case 'Pending': return 'bg-orange-500/10 text-orange-500 border-orange-500/20';
      case 'Inactive': return 'bg-[#6fa3a0]/10 text-[#6fa3a0] border-[#6fa3a0]/20';
      case 'Rejected': return 'bg-red-500/10 text-red-600 border-red-500/20';
      case 'New': return 'bg-blue-500/10 text-[#D2B48C] border-[#D2B48C]/20';
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 self-start md:self-auto">
          ...
          <button 
            ...
            className="px-4 py-2 bg-[#164A4A] text-white font-bold rounded-xl hover:bg-[#C6A77D] transition-colors flex items-center gap-2 shadow-lg shadow-[#164A4A]/20"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA] flex flex-col justify-center">
          <div className="flex items-center space-x-2 text-[#455250] mb-2"><Users size={16}/> <span className="font-semibold text-sm">Total</span></div>
          <span className="text-2xl font-bold text-[#202828]">{members.length}</span>
        </div>
        <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA] flex flex-col justify-center">
          <div className="flex items-center space-x-2 text-[#164A4A] mb-2"><CheckCircle size={16}/> <span className="font-semibold text-sm">Active</span></div>
          <span className="text-2xl font-bold text-[#202828]">{members.filter(m => m.status === 'Active').length}</span>
        </div>
        <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA] flex flex-col justify-center">
          ...
          <span className="text-2xl font-bold text-[#202828]">{members.filter(m => m.status === 'Pending').length}</span>
        </div>
        <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA] flex flex-col justify-center">
          <div className="flex items-center space-x-2 text-[#6fa3a0] mb-2"><XCircle size={16}/> <span className="font-semibold text-sm">Inactive</span></div>
          <span className="text-2xl font-bold text-[#202828]">{members.filter(m => m.status === 'Inactive').length}</span>
        </div>
        <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA] flex flex-col justify-center">
          ...
          <span className="text-2xl font-bold text-[#202828]">{members.filter(m => m.customerType === 'NEW_CUSTOMER').length}</span>
        </div>
      </div>

      {/* Filters & Search */}
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-center bg-[#FFFFFF] p-4 rounded-2xl border border-[#D3DFDA]">
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          {['All', 'Active', 'Inactive', 'Pending', 'Rejected', 'Existing', 'New'].map(f => (
            <button 
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-colors ${filter === f ? 'bg-[#164A4A] text-white' : 'bg-[#F2EFE8] text-[#455250] hover:bg-[#E8E5DA]'}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="relative w-full lg:w-72">
          <input 
            ...
            className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl pl-10 pr-4 py-2 text-[#202828] outline-none focus:border-[#164A4A] text-sm"
          />
                ...
                <tr key={`${member.id}-${index}`} className="hover:bg-[#F1F5F3] transition-colors relative">
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-[#202828] font-bold capitalize">{member.name}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[#FFFFFF] rounded-2xl p-6 md:p-8 max-w-md w-full shadow-2xl border border-[#D3DFDA] text-center">
            <ShieldCheck size={60} className="mx-auto text-[#164A4A] mb-4" />
            <h2 className="text-2xl font-bold text-[#202828] mb-2">Member Limit Reached</h2>
            <p className="text-[#455250] mb-6">Your current subscription plan limits you to {getMemberLimit(user?.subscriptionPlan)} members. Upgrade your plan to add more members and grow your gym!</p>
            <div className="flex flex-col space-y-3">
              <button onClick={() => navigate('/admin/subscription')} className="w-full py-3 bg-[#164A4A] text-white font-bold rounded-xl hover:bg-[#C6A77D] transition-colors shadow-lg shadow-[#164A4A]/20">
                View Upgrade Options
              </button>
              <button onClick={() => setShowUpgradeModal(false)} className="w-full py-3 bg-[#F2EFE8] text-[#455250] font-bold rounded-xl hover:bg-[#F1F5F9] transition-colors">
                Maybe Later
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#FFFFFF] rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-[#D3DFDA] my-8 shrink-0">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-[#202828] mb-1 capitalize">{selectedMember.name}</h3>
                  <div className="flex gap-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${selectedMember.status === 'Active' ? 'bg-[#164A4A]/10 text-[#164A4A]' : 'bg-[#6fa3a0]/10 text-[#6fa3a0]'}`}>
                      {selectedMember.status}
                    </span>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${selectedMember.customerType === 'NEW_CUSTOMER' ? 'bg-blue-500/10 text-[#D2B48C]' : 'bg-gray-500/10 text-gray-600'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                <div>
                  ...
                  <p className="font-semibold text-[#202828]">{selectedMember.email}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Phone</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.phone}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#202828]">{selectedMember.joined}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.lastLogin ? new Date(selectedMember.originalUser.lastLogin).toLocaleString() : 'Never'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Date of Birth</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.dob || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">City & PIN</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.city || 'Not specified'} - {selectedMember.originalUser?.pinCode || ''}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Height / Weight</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.height ? `${selectedMember.originalUser.height} cm` : '--'} / {selectedMember.originalUser?.weight ? `${selectedMember.originalUser.weight} kg` : '--'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Fitness Goal</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.fitnessGoal || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Experience Level</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.experienceLevel || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Pref. Training</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.preferredTraining || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Pref. Workout Time</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.originalUser?.preferredWorkoutTime || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#455250] uppercase tracking-wider mb-1">Assigned Trainer</p>
                  <p className="font-semibold text-[#202828]">{selectedMember.trainer || 'Unassigned'}</p>
                </div>
              </div>

              <div className="pt-2 border-t border-[#D3DFDA]">
                <p className="text-[#164A4A] font-bold mb-4 uppercase text-xs tracking-wider flex items-center gap-2"><Phone size={14}/> Emergency Contact</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <p className="text-xs font-bold text-[#455250] mb-1">Name</p>
                    <p className="font-semibold text-[#202828]">{selectedMember.emergencyName || 'None provided'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-[#455250] mb-1">Phone</p>
                    <p className="font-semibold text-[#202828]">{selectedMember.emergencyPhone || 'None provided'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-[#455250] mb-1">Relationship</p>
                    <p className="font-semibold text-[#202828]">{selectedMember.emergencyRelation || 'None provided'}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 bg-[#F2EFE8] border-t border-[#D3DFDA] flex justify-end gap-3">
              ...
              <button onClick={() => setSelectedMember(null)} className="px-6 py-2 bg-[#FFFFFF] border border-[#E8E5DA] text-[#202828] rounded-xl font-bold hover:bg-[#F1F5F9] transition-colors">
                Close Profile
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#FFFFFF] rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-[#D3DFDA] my-8 shrink-0">
            <div className="p-6 border-b border-[#D3DFDA] flex justify-between items-center bg-[#F2EFE8]">
              ...
              <button onClick={() => setEditMember(null)} className="text-[#455250] hover:text-[#202828] transition-colors p-2 hover:bg-gray-100 rounded-lg">
                <XCircle size={24} />
              </button>
            </div>
            <form onSubmit={handleEditSave}>
              <div className="p-6 space-y-6">
                ...
                {/* Personal Info */}
                <div>
                  <p className="text-xs font-bold text-[#164A4A] uppercase tracking-wider mb-3 flex items-center gap-2"><User size={14}/> Personal Information</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Full Name *</label>
                      <input required value={editForm.name || ''} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="Full Name" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Email Address *</label>
                      <input required type="email" value={editForm.email || ''} onChange={e => setEditForm({...editForm, email: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="Email" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Phone Number *</label>
                      <input required type="tel" value={editForm.phone || ''} onChange={e => { const v = e.target.value.replace(/\D/g,''); if(v.length<=10) setEditForm({...editForm, phone: v}); }} maxLength={10} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="10-digit number" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Gender</label>
                      <select value={editForm.gender || 'Male'} onChange={e => setEditForm({...editForm, gender: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm">
                        <option>Male</option>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Date of Birth</label>
                      <input type="date" value={editForm.dob || ''} onChange={e => setEditForm({...editForm, dob: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Fitness Goal</label>
                      <select value={editForm.fitnessGoal || 'General Fitness'} onChange={e => setEditForm({...editForm, fitnessGoal: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm">
                ...
                {/* Membership */}
                <div className="border-t border-[#D3DFDA] pt-6">
                  <p className="text-xs font-bold text-[#164A4A] uppercase tracking-wider mb-3 flex items-center gap-2"><ShieldCheck size={14}/> Membership</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Plan</label>
                      <select value={editForm.plan || ''} onChange={e => setEditForm({...editForm, plan: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm">
                        {mockPlans.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Status</label>
                      <select value={editForm.status || 'Active'} onChange={e => setEditForm({...editForm, status: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Join Date</label>
                      <input type="date" value={editForm.joined || ''} onChange={e => setEditForm({...editForm, joined: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Assigned Trainer</label>
                      <input value={editForm.trainer || ''} onChange={e => setEditForm({...editForm, trainer: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="Trainer Name" />
                    </div>
                  </div>
                </div>
                ...
                {/* Emergency Contact */}
                <div className="border-t border-[#D3DFDA] pt-6">
                  <p className="text-xs font-bold text-[#164A4A] uppercase tracking-wider mb-3 flex items-center gap-2"><Phone size={14}/> Emergency Contact</p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Name</label>
                      <input value={editForm.emergencyName || ''} onChange={e => setEditForm({...editForm, emergencyName: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="Contact Name" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Phone</label>
                      <input type="tel" value={editForm.emergencyPhone || ''} onChange={e => { const v = e.target.value.replace(/\D/g,''); if(v.length<=10) setEditForm({...editForm, emergencyPhone: v}); }} maxLength={10} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="Phone" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#455250] mb-1">Relationship</label>
                      <input value={editForm.emergencyRelation || ''} onChange={e => setEditForm({...editForm, emergencyRelation: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#D3DFDA] rounded-xl px-4 py-2.5 outline-none focus:border-[#164A4A] text-sm" placeholder="e.g. Parent" />
                    </div>
                  </div>
                </div>
              </div>
