...
const GymAdminProfile = () => {
  ...
  const inputCls = `w-full rounded-xl px-4 py-3 text-[#202522] outline-none transition-colors ${
    isEditing 
      ? 'bg-[#FFFFFF] border border-[#DCD9CD] focus:border-[#34483F]' 
      : 'bg-[#F2EFE8] border border-transparent cursor-not-allowed text-opacity-80'
  }`;
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#202522] tracking-tight">Gym Profile</h1>
          <p className="text-[#4A514D] mt-1">Manage your facility's public information and details.</p>
        </div>
        {/* Top-level Edit toggle */}
        {!isEditing && (
          <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-5 py-2.5 bg-[#202522] text-white font-bold rounded-xl hover:bg-[#0F172A] transition-colors shadow-lg text-sm">
      ...
      {/* Header Banner */}
      <div className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl p-6 md:p-8 flex items-center space-x-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#34483F]/5 rounded-full blur-3xl -mr-20 -mt-20"></div>
        <div className="w-24 h-24 bg-[#FFFFFF] border-2 border-[#34483F] rounded-2xl flex items-center justify-center text-[#34483F] shrink-0 z-10 overflow-hidden">
          {gym?.logo ? <img src={gym.logo} alt="Logo" className="w-full h-full object-cover" /> : <Building2 size={40} />}
        </div>
        <div className="z-10">
          <h2 className="text-2xl font-bold text-[#202522]">{formData.gymName || 'Loading...'}</h2>
          <p className="text-[#34483F] font-semibold flex items-center mt-1"><MapPin size={16} className="mr-1"/> {formData.city}, {formData.state}</p>
        </div>
      </div>
      ...
      <div className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl p-6 md:p-8 space-y-8">
        ...
        {/* Basic Info */}
        <section>
          <div className="flex items-center space-x-2 text-[#202522] mb-4 border-b border-[#DCD9CD] pb-2">
            <Info size={20} className="text-[#34483F]"/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Facility Type</label>
              <input disabled={!isEditing} value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className={inputCls} />
            </div>
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Contact Email</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Contact Phone</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm text-[#4A514D] mb-2">About the Gym</label>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Owner Name</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Role</label>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Website</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Tax ID / Registration Number</label>
              <input disabled={!isEditing} value={formData.taxId} onChange={e => setFormData({...formData, taxId: e.target.value})} className={inputCls} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm text-[#4A514D] mb-2">Standard Operating Hours</label>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm text-[#4A514D] mb-2">Street Address</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">State / Province</label>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Established Year</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Training Mode</label>
            ...
            <div>
              <label className="block text-sm text-[#4A514D] mb-2">Total Trainers</label>
            ...
            {gym?.services && gym.services.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {gym.services.map((service: string, i: number) => (
                  <span key={i} className="px-3 py-1 bg-[#34483F]/10 text-[#34483F] font-semibold text-sm rounded-lg">
                    {service}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-sm text-[#4A514D]">No services listed</p>
            )}
          </div>
          <div>
            <div className="flex items-center space-x-2 text-[#202522] mb-4 border-b border-[#DCD9CD] pb-2">
            ...
            {gym?.facilities && gym.facilities.length > 0 ? (
              ...
              <p className="text-sm text-[#4A514D]">No facilities listed</p>
            )}
          </div>
        </section>
              ...
              {gym.acDetails.areas && gym.acDetails.areas.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {gym.acDetails.areas.map((area: string, i: number) => (
                    <span key={i} className="px-3 py-1 bg-[#F2EFE8] border border-[#E8E5DA] text-[#4A514D] font-medium text-sm rounded-lg">
                      {area}
                    </span>
                  ))}
                </div>
        ...
        <div className="pt-4 flex justify-end">
          {isEditing && (
            <div className="flex gap-4">
              <button onClick={() => setIsEditing(false)} disabled={isSaving} className="px-6 py-3 bg-[#F1F5F9] text-[#4A514D] font-bold rounded-xl hover:bg-[#E8E5DA] transition-colors disabled:opacity-50">
                Cancel
              </button>
              <button onClick={handleSave} disabled={isSaving} className="px-6 py-3 bg-[#34483F] text-white font-bold rounded-xl hover:bg-[#C6A77D] transition-colors flex items-center gap-2 shadow-lg shadow-[#34483F]/20 disabled:opacity-70">
