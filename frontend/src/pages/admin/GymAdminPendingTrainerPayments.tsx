...
const GymAdminPendingTrainerPayments = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-5 right-5 z-50 px-5 py-3 rounded-xl shadow-xl font-semibold text-white text-sm flex items-center gap-2 ${toast.type === 'success' ? 'bg-[#164A4A]' : 'bg-red-500'}`}>
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#202828]">Pending Trainer Payments</h1>
        <p className="text-[#687B78] text-sm mt-1">Review and process outstanding trainer payments</p>
      </div>
      ...
      <div className="bg-white border border-[#E8E5DA] rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-16 px-6">
            ...
            <p className="text-[#202828] font-bold text-lg">No pending payments</p>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-[#F2EFE8] border-b border-[#E8E5DA]">
                  ...
                  <th className="px-5 py-3.5 font-semibold text-[#687B78] text-right">Fee</th>
                  <th className="px-5 py-3.5 font-semibold text-[#687B78]">Billing Cycle</th>
                  <th className="px-5 py-3.5 font-semibold text-[#687B78]">Due Date</th>
                  <th className="px-5 py-3.5 font-semibold text-[#687B78] text-right">Amount Due</th>
                  ...
                  <tr key={i} className="hover:bg-[#F2EFE8] transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-[#202828]">{trainerName(item.trainer)}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-right font-bold text-[#202828]">₹{item.feeAmount?.toLocaleString('en-IN')}{cycleSuffix(item.billingCycle)}</td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-right font-bold text-[#202828]">₹{item.amount?.toLocaleString('en-IN')}</td>
                    <td className="px-5 py-4">
                      {(() => {
                        const currentStatus = statuses[item._id] || item.status || 'Pending';
                        let colors = 'bg-amber-100 text-amber-700 focus:ring-amber-200';
                        if (currentStatus === 'Active') colors = 'bg-[#D2B48C]/10 text-[#164A4A] focus:ring-green-200';
                    ...
                    <td className="px-5 py-4">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-1.5 bg-[#164A4A] text-white rounded-lg text-xs font-bold hover:bg-[#C6A77D] transition-colors shadow shadow-green-200"
                      >
                        <CreditCard size={13} />
                        Process Payment
                      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-black/50 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-[#D3DFDA] mt-10 mb-10">
            <div className="p-6 border-b border-[#D3DFDA] flex justify-between items-center bg-gradient-to-r from-[#F1F5F3] to-[#FFFFFF]">
              <div>
                <h2 className="text-xl font-bold text-[#202828]">Process Payment</h2>
                <p className="text-sm text-[#687B78] mt-1">
                  Recording payment for <span className="font-semibold text-[#164A4A]">{trainerName(selectedTrainer.trainer)}</span>
                </p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-[#F1F5F3] border border-[#D3DFDA] rounded-xl p-4 flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <p className="font-semibold text-[#202828]">{selectedTrainer.billingCycle}</p>
                </div>
              </div>

              {/* Payment Method */}
              <div>
                <label className="block text-sm font-semibold text-[#455250] mb-2">Payment Method</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 text-xs font-semibold transition-all ${payForm.paymentMethod === m.value ? 'border-[#164A4A] bg-[#F1F5F3] text-[#164A4A]' : 'border-[#E8E5DA] text-[#687B78] hover:border-[#D3DFDA]'}`}
                    >
                      <m.icon size={18} />
                      {m.label}
                    </button>
              ...
              {payForm.paymentMethod === 'Bank Transfer' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[#455250] mb-1">Account Holder</label>
                    <input type="text" value={payForm.accountHolder} onChange={e => setPayForm({ ...payForm, accountHolder: e.target.value })} className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#455250] mb-1">Bank Name</label>
                    <input type="text" value={payForm.bankName} onChange={e => setPayForm({ ...payForm, bankName: e.target.value })} className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                  </div>
                  <div>
              ...
              {payForm.paymentMethod === 'UPI' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-[#455250] mb-1">UPI ID</label>
                    <input type="text" value={payForm.upiId} onChange={e => setPayForm({ ...payForm, upiId: e.target.value })} placeholder="name@bank" className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#455250] mb-1">Payment Name</label>
                    <input type="text" value={payForm.upiName} onChange={e => setPayForm({ ...payForm, upiName: e.target.value })} className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#455250] mb-1">Amount (₹) *</label>
                  <input type="number" value={payForm.amount} onChange={e => setPayForm({ ...payForm, amount: e.target.value })} className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#455250] mb-1">Payment Date *</label>
                  <input type="date" value={payForm.paymentDate} onChange={e => setPayForm({ ...payForm, paymentDate: e.target.value })} className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A]" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#455250] mb-1">Notes</label>
                <textarea value={payForm.notes} onChange={e => setPayForm({ ...payForm, notes: e.target.value })} rows={2} placeholder="Optional notes..." className="w-full border border-[#E8E5DA] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#164A4A] resize-none" />
              </div>
            </div>
            <div className="p-6 border-t border-[#E8E5DA] flex gap-3">
              <button onClick={() => setSelectedTrainer(null)} className="flex-1 py-2.5 border border-[#E8E5DA] rounded-xl text-sm font-semibold text-[#687B78] hover:bg-[#F2EFE8] transition-colors">
                Cancel
              </button>
              <button onClick={handleProcess} disabled={saving} className="flex-1 py-2.5 bg-[#164A4A] text-white rounded-xl text-sm font-bold hover:bg-[#C6A77D] transition-colors disabled:opacity-60 shadow-lg shadow-green-200">
