      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <BarChart className="text-[#34483F]" size={32} />
            Reports & Analytics
          </h1>
          <p className="text-[#4A514D] mt-2">Generate custom CSV exports for accounting and BI.</p>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {['Financial End-of-Month', 'User Growth & Retention', 'Trainer Payout Ledger', 'AI Token Usage', 'Gym Check-ins Report'].map((report, i) => (
          <div key={i} className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl p-6 flex flex-col justify-between">
            <div>
              <div className="w-10 h-10 bg-[#E8E5DA] rounded-lg mb-4 flex items-center justify-center text-[#4A514D]">
                <BarChart size={20} />
              </div>
              <h3 className="font-bold mb-2">{report}</h3>
              <p className="text-sm text-[#4A514D] mb-6">Standard CSV export for BI tools.</p>
            </div>
