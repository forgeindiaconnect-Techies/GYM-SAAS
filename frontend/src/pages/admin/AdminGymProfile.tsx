...
const AdminGymProfile = () => {
  ...
  if (!gym) {
    return <div className="text-center py-20 text-[#475569]">No gym profile found.</div>;
  }

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-4 mb-8">
        <div className="w-20 h-20 bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl flex items-center justify-center overflow-hidden shrink-0">
          {gym.logo ? (
        ...
        <div>
          <h1 className="text-3xl font-bold text-[#1E293B]">{gym.name}</h1>
          <div className="flex items-center gap-3 mt-2">
            <span className="text-[#16A34A] font-medium">{gym.gymType || 'Fitness Facility'}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-[#555]"></span>
            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${
              gym.status === 'ACTIVE' ? 'bg-[#16A34A]/10 text-[#16A34A]' :
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-[#CCFBF1] pb-3">
            <Phone className="text-[#16A34A]" size={20} />
            Contact Details
          </h2>
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <Mail className="text-[#475569] mt-1 shrink-0" size={16} />
              <div>
                <p className="text-sm text-[#475569]">Official Email</p>
                <p className="text-[#1E293B] font-medium">{gym.email}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <Phone className="text-[#475569] mt-1 shrink-0" size={16} />
              <div>
                <p className="text-sm text-[#475569]">Phone Number</p>
                <p className="text-[#1E293B] font-medium">{gym.phone}</p>
              </div>
            </div>
            {gym.website && (
              <div className="flex items-start gap-3">
                <Globe className="text-[#475569] mt-1 shrink-0" size={16} />
                <div>
                  <p className="text-sm text-[#475569]">Website</p>
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <MapPin className="text-[#475569] mt-1 shrink-0" size={16} />
              <div>
                <p className="text-sm text-[#475569]">Full Address</p>
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6 md:col-span-2">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-[#CCFBF1] pb-3">
            <CreditCard className="text-[#16A34A]" size={20} />
            Platform Subscription
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-sm text-[#475569]">Plan Tier</p>
              <p className="text-[#1E293B] font-bold text-lg mt-1">{gym.subscription?.plan || 'N/A'}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-sm text-[#475569]">Start Date</p>
              <p className="text-[#1E293B] font-medium mt-1">
                {gym.subscription?.startDate ? new Date(gym.subscription.startDate).toLocaleDateString() : 'N/A'}
              </p>
            </div>
            <div>
              <p className="text-sm text-[#475569]">End Date</p>
              <p className="text-[#1E293B] font-medium mt-1">
        ...
        {gym.description && (
          <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6 md:col-span-2">
            <h2 className="text-xl font-bold mb-4 border-b border-[#CCFBF1] pb-3">About the Gym</h2>
            <p className="text-[#475569] leading-relaxed">{gym.description}</p>
          </div>
        )}
      </div>
    </div>
