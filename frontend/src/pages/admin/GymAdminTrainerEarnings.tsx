...
const GymAdminTrainerEarnings = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#202522]">Trainer Earnings Overview</h1>
        <p className="text-[#727975] text-sm mt-1">Summary of all trainer fee configurations and payments</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-[#E8E5DA] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#727975] uppercase tracking-wide">Total Paid Out</p>
          </div>
          <p className="text-2xl font-bold text-[#202522]">₹{(data?.totalPaidOut || 0).toLocaleString('en-IN')}</p>
        ...
        <div className="bg-white border border-[#E8E5DA] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#727975] uppercase tracking-wide">Active Trainers</p>
          </div>
          <p className="text-2xl font-bold text-[#202522]">{fees.length}</p>
        ...
        <div className="bg-white border border-[#E8E5DA] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#727975] uppercase tracking-wide">Monthly Commitment</p>
          </div>
          <p className="text-2xl font-bold text-[#202522]">₹{totalConfiguredMonthly.toLocaleString('en-IN')}</p>
        ...
        <div className="bg-white border border-[#E8E5DA] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#727975] uppercase tracking-wide">Avg Payout</p>
          </div>
          <p className="text-2xl font-bold text-[#202522]">
      ...
      <div className="bg-white border border-[#E8E5DA] rounded-2xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-[#E8E5DA]">
          <h2 className="font-bold text-[#202522]">Trainer Earnings Breakdown</h2>
        </div>
        {trainerSummary.length === 0 ? (
          <div className="text-center py-16 px-6">
            ...
            <p className="text-[#202522] font-bold text-lg">No trainer fee data yet</p>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-[#F2EFE8] border-b border-[#E8E5DA]">
                  ...
                  <th className="px-5 py-3.5 font-semibold text-[#727975] text-right">Configured Fee</th>
                  <th className="px-5 py-3.5 font-semibold text-[#727975]">Billing Cycle</th>
                  <th className="px-5 py-3.5 font-semibold text-[#727975] text-right">Total Paid</th>
                  <th className="px-5 py-3.5 font-semibold text-[#727975]">Payments</th>
                </tr>
              </thead>
                  ...
                  <tr key={fee._id} className="hover:bg-[#F2EFE8] transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-[#202522]">{fee.trainerId?.name || 'Unknown'}</p>
                          <p className="text-xs text-[#A8ADA9]">{fee.trainerId?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-[#4A514D]">{fee.trainingType}</td>
                    <td className="px-5 py-4 text-right font-bold text-[#202522]">₹{fee.feeAmount?.toLocaleString('en-IN')}</td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-[#727975]">{paymentsCount} payments</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between p-5 border-b border-[#E8E5DA] bg-[#F2EFE8]">
              <div>
                <h3 className="text-lg font-bold text-[#202522]">{selectedTrainer.name}'s Payments</h3>
                <p className="text-xs text-[#727975]">{selectedTrainer.email}</p>
              </div>
              <button
                onClick={() => setSelectedTrainer(null)}
                className="text-[#727975] hover:text-[#202522] bg-white rounded-full p-1 border border-gray-200"
              >
                ✕
              </button>
            </div>
                ...
                if (trainerPayments.length === 0) {
                  return (
                    <div className="text-center py-10">
                      <p className="text-[#727975]">No payments recorded for this trainer yet.</p>
                    </div>
                  );
                }
                      ...
                      <div key={i} className="bg-white border border-[#E8E5DA] rounded-xl overflow-hidden shadow-sm hover:border-[#CBD5E1] transition-colors">
                        <div className="flex justify-between items-center bg-[#F2EFE8] p-4 border-b border-[#E8E5DA]">
                          <div>
                            ...
                            <span className="text-xl font-bold text-[#202522]">₹{p.amount?.toLocaleString('en-IN')}</span>
                          </div>
