...
const GymAdminTrainerEarnings = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#1E293B]">Trainer Earnings Overview</h1>
        <p className="text-[#64748B] text-sm mt-1">Summary of all trainer fee configurations and payments</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Total Paid Out</p>
          </div>
          <p className="text-2xl font-bold text-[#1E293B]">₹{(data?.totalPaidOut || 0).toLocaleString('en-IN')}</p>
          <p className="text-xs text-[#94A3B8] mt-1">{data?.paymentsCount || 0} payments</p>
        </div>
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Active Trainers</p>
          </div>
          <p className="text-2xl font-bold text-[#1E293B]">{fees.length}</p>
          <p className="text-xs text-[#94A3B8] mt-1">With active fee config</p>
        </div>
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Monthly Commitment</p>
          </div>
          <p className="text-2xl font-bold text-[#1E293B]">₹{totalConfiguredMonthly.toLocaleString('en-IN')}</p>
          <p className="text-xs text-[#94A3B8] mt-1">Monthly trainers only</p>
        </div>
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#64748B] uppercase tracking-wide">Avg Payout</p>
          </div>
          <p className="text-2xl font-bold text-[#1E293B]">
            ₹{data?.paymentsCount ? Math.round((data?.totalPaidOut || 0) / data.paymentsCount).toLocaleString('en-IN') : '0'}
          </p>
          <p className="text-xs text-[#94A3B8] mt-1">Per payment</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-[#E2E8F0]">
          <h2 className="font-bold text-[#1E293B]">Trainer Earnings Breakdown</h2>
        </div>
        {trainerSummary.length === 0 ? (
          <div className="text-center py-16 px-6">
            ...
            <p className="text-[#1E293B] font-bold text-lg">No trainer fee data yet</p>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                  ...
                  <th className="px-5 py-3.5 font-semibold text-[#64748B] text-right">Configured Fee</th>
                  <th className="px-5 py-3.5 font-semibold text-[#64748B]">Billing Cycle</th>
                  <th className="px-5 py-3.5 font-semibold text-[#64748B] text-right">Total Paid</th>
                  <th className="px-5 py-3.5 font-semibold text-[#64748B]">Payments</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#F1F5F9]">
                {trainerSummary.map(({ fee, paid, paymentsCount }) => (
                  <tr key={fee._id} className="hover:bg-[#F8FAFC] transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-[#1E293B]">{fee.trainerId?.name || 'Unknown'}</p>
                          <p className="text-xs text-[#94A3B8]">{fee.trainerId?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-[#475569]">{fee.trainingType}</td>
                    <td className="px-5 py-4 text-right font-bold text-[#1E293B]">₹{fee.feeAmount?.toLocaleString('en-IN')}</td>
                    <td className="px-5 py-4">
                      <span className="px-2.5 py-1 rounded-lg bg-blue-100 text-blue-700 text-xs font-semibold">{fee.billingCycle}</span>
                    </td>
                    <td className="px-5 py-4 text-right font-bold text-green-600">₹{paid.toLocaleString('en-IN')}</td>
                    <td className="px-5 py-4 text-[#64748B]">{paymentsCount} payments</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0] bg-[#F8FAFC]">
              <div>
                <h3 className="text-lg font-bold text-[#1E293B]">{selectedTrainer.name}'s Payments</h3>
                <p className="text-xs text-[#64748B]">{selectedTrainer.email}</p>
              </div>
              <button
                onClick={() => setSelectedTrainer(null)}
                className="text-[#64748B] hover:text-[#1E293B] bg-white rounded-full p-1 border border-gray-200"
              >
                ✕
              </button>
            </div>
                ...
                if (trainerPayments.length === 0) {
                  return (
                    <div className="text-center py-10">
                      <p className="text-[#64748B]">No payments recorded for this trainer yet.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="space-y-4">
                    {trainerPayments.map((p, i) => (
                      <div key={i} className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden shadow-sm hover:border-[#CBD5E1] transition-colors">
                        <div className="flex justify-between items-center bg-[#F8FAFC] p-4 border-b border-[#E2E8F0]">
                          <div>
                            <span className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1 block">Payment Amount</span>
                            <span className="text-xl font-bold text-[#1E293B]">₹{p.amount?.toLocaleString('en-IN')}</span>
                          </div>
