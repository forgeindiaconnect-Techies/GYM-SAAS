  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#202828] tracking-tight">My Enquiries</h1>
          <p className="text-[#455250] mt-1">Track the status of your queries sent to gyms.</p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#F2EFE8] border-b border-[#E8E5DA]">
                  <th className="px-6 py-4 text-xs font-semibold text-[#687B78] uppercase tracking-wider">Gym Details</th>
                  <th className="px-6 py-4 text-xs font-semibold text-[#687B78] uppercase tracking-wider">Enquiry Info</th>
                  <th className="px-6 py-4 text-xs font-semibold text-[#687B78] uppercase tracking-wider">Date Submitted</th>
                ...
                {filteredEnquiries.map((enq) => (
                  <tr key={enq._id} className="hover:bg-[#F2EFE8] transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-[#202828] flex items-center gap-1.5">
                        <Building2 size={14} className="text-[#164A4A]"/> {enq.gymId?.name || 'Unknown Gym'}
                      </div>
                      <div className="text-sm text-[#455250] flex items-center gap-1 mt-1">
                        <MapPin size={12}/> {enq.branchId?.name || enq.city || 'Main Branch'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm font-semibold text-[#202828]">{enq.enquiryType}</div>
                      <div className="text-sm text-[#687B78] line-clamp-1 max-w-xs">{enq.message}</div>
                    </td>
                    <td className="px-6 py-4">
