  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-[#4A514D]">Your personalized meal plan</p>
      </div>
        ...
        {[
          ...
          <div key={i} className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-medium text-[#4A514D]">{macro.label}</span>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-bold">{macro.value}</span>
              <span className="text-[#4A514D]">/ {macro.target}{macro.unit}</span>
            </div>
      ...
      <div className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-[#DCD9CD] flex justify-between items-center">
          ...
          <button className="text-sm text-[#34483F] hover:underline">Generate New Plan</button>
        </div>
          ...
          {[
            ...
            <div key={i} className="p-6 flex items-center justify-between hover:bg-[#FFFFFF] transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-[#E8E5DA] rounded-xl flex items-center justify-center text-[#4A514D]">
                  <m.icon size={20} />
                </div>
                <div>
                  <h3 className="font-bold text-lg">{m.meal} <span className="text-xs font-normal text-[#4A514D] ml-2">{m.time}</span></h3>
                  <p className="text-sm text-[#4A514D] mt-1">{m.items}</p>
                </div>
              </div>
              <div className="text-right">
                <span className="font-bold text-[#34483F]">{m.cal}</span>
                <span className="text-xs text-[#4A514D] ml-1">kcal</span>
              </div>
            </div>
          ))}
        </div>
