...
const MemberProfile = () => {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-full p-12 text-[#64748B]">Loading profile...</div>;
  }
      ...
      <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 relative z-10">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-[#1E293B] mb-1">{displayUser?.firstName || 'User'} {displayUser?.lastName || ''}</h1>
            <p className="text-[#16A34A] font-medium capitalize mb-4">{displayUser?.subscriptionPlan || 'Member'} Plan</p>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#F1F5F9] text-[#1E293B] font-semibold hover:bg-[#E2E8F0] rounded-xl transition-colors"
          >
        ...
        <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {displayUser?.fitnessGoal ? (
              <div className="p-4 bg-[#FFFFFF] rounded-xl border border-[#CCFBF1]">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium text-[#16A34A]">Current Goal</span>
                </div>
                <p className="text-lg text-[#1E293B] font-semibold">{displayUser.fitnessGoal}</p>
                <p className="text-sm text-[#475569] mt-2">Target set during onboarding.</p>
              </div>
            ) : (
              <div className="p-4 bg-[#F8FAFC] rounded-xl border border-dashed border-[#CBD5E1] text-center">
                <p className="text-[#64748B] mb-2">No specific fitness goal set.</p>
                <button onClick={handleEditClick} className="text-sm text-[#16A34A] font-bold hover:underline">Set Your Goal</button>
              </div>
            )}
            ...
            {displayUser?.experienceLevel && (
              <div className="p-4 bg-[#FFFFFF] rounded-xl border border-[#CCFBF1]">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium text-[#1E293B]">Experience Level</span>
                </div>
                <p className="text-sm text-[#475569] mt-1 font-medium">{displayUser.experienceLevel}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white p-6 border-b border-[#E2E8F0] flex justify-between items-center z-10">
              <h2 className="text-2xl font-bold text-[#1E293B]">Edit Profile</h2>
              <button 
                onClick={() => setIsEditing(false)}
                className="p-2 hover:bg-[#F1F5F9] rounded-full transition-colors text-[#64748B] hover:text-[#1E293B]"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl px-4 py-3 focus:border-[#16A34A] focus:ring-2 focus:ring-[#16A34A]/20 outline-none transition-all"
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#1E293B] mb-2">Mobile</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#1E293B] mb-2">Date of Birth</label>
                  <input
                    type="date"
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    <label className="block text-sm font-semibold text-[#1E293B] mb-2">Height (cm)</label>
                    <input
                      type="number"
                  ...
                  <div className="flex-1">
                    <label className="block text-sm font-semibold text-[#1E293B] mb-2">Weight (kg)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#1E293B] mb-2">Pincode</label>
                  <input
                    type="text"
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-semibold text-[#1E293B] mb-2">Fitness Goal</label>
                  <input
                    type="text"
              ...
              <div className="pt-6 border-t border-[#E2E8F0] flex justify-end gap-4">
                <button 
                  type="button"
                  onClick={() => setIsEditing(false)}
                  className="px-6 py-3 bg-[#F1F5F9] text-[#1E293B] font-bold rounded-xl hover:bg-[#E2E8F0] transition-colors"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  disabled={isSaving}
                  className="flex items-center gap-2 px-6 py-3 bg-[#16A34A] text-white font-bold rounded-xl hover:bg-[#15803D] transition-colors shadow-lg shadow-green-500/30 disabled:opacity-70"
                >
                  {isSaving ? (
                    'Saving...'
                  ) : (
