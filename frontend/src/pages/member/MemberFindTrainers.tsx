...
const MemberFindTrainers = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="text-center py-12 text-[#475569]">Loading trainers...</div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(trainer => (
              <div key={trainer._id} className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6 hover:border-[#16A34A]/50 transition-colors flex flex-col h-full">
                <div className="flex items-start space-x-4">
                  <div className="w-16 h-16 bg-[#FFFFFF] rounded-full flex items-center justify-center text-xl font-bold text-[#16A34A] border border-[#CCFBF1] overflow-hidden">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[#16A34A] font-medium">{trainer.specialization || 'General Fitness'}</p>
                ...
                <div className="mt-4 pt-4 border-t border-[#CCFBF1] space-y-2 text-sm text-[#475569] flex-1">
                  <p><span className="text-[#1E293B]">Experience:</span> {trainer.experience ? `${trainer.experience} years` : 'Not specified'}</p>
                  <p><span className="text-[#1E293B]">Availability:</span> {trainer.availability || 'Weekdays'}</p>
                  {trainer.trainingMode && (
                    <p><span className="text-[#1E293B]">Mode:</span> <span className="capitalize">{trainer.trainingMode}</span></p>
                  )}
                </div>
                
                <div className="mt-6 flex gap-3">
                  <button onClick={() => setSelectedTrainer(trainer)} className="flex-1 flex justify-center items-center space-x-2 py-2 bg-[#F1F5F9] text-[#1E293B] border border-[#CCFBF1] rounded-xl font-semibold hover:bg-[#E2E8F0] transition-colors">
                    <Eye size={16} />
                    <span>View Details</span>
                  </button>
                  <Link to={`/member/book-session?trainerId=${trainer._id}`} className="flex-1 flex justify-center items-center space-x-2 py-2 bg-[#16A34A] text-white rounded-xl font-semibold hover:bg-[#15803D] transition-colors">
                    <Calendar size={16} />
      ...
      {selectedTrainer && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar animate-scale-in flex flex-col shadow-2xl border border-[#CCFBF1]">
            <div className="flex justify-between items-center p-6 border-b border-[#CCFBF1] sticky top-0 bg-white/95 backdrop-blur z-10">
              <h2 className="text-xl font-bold text-[#1E293B]">Trainer Profile</h2>
              <button onClick={() => setSelectedTrainer(null)} className="text-[#475569] hover:bg-[#F1F5F9] p-2 rounded-full transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 md:p-8 space-y-8">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                <div className="w-32 h-32 bg-[#F1F5F9] rounded-2xl flex items-center justify-center text-4xl font-bold text-[#16A34A] border-4 border-[#CCFBF1] overflow-hidden shadow-lg shrink-0">
                ...
                <div className="text-center md:text-left flex-1">
                  <h3 className="text-3xl font-bold text-[#1E293B] mb-2">{selectedTrainer.name}</h3>
                  <p className="text-lg text-[#16A34A] font-semibold mb-3">{selectedTrainer.specialization || 'General Fitness'}</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4">
                    ...
                    <p className="font-semibold text-[#1E293B]">{selectedTrainer.email || 'N/A'}</p>
                  </div>
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4">
                    ...
                    <p className="font-semibold text-[#1E293B]">{selectedTrainer.availableDays || 'N/A'}</p>
                  </div>
                  <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4">
                  ...
                  {selectedTrainer.availableSlot && (
                    <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4 sm:col-span-2">
                      ...
                      <p className="font-semibold text-[#1E293B]">{selectedTrainer.availableSlot}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-6 border-t border-[#CCFBF1] bg-[#F8FAFC] flex justify-end gap-3 sticky bottom-0 rounded-b-3xl">
              <button onClick={() => setSelectedTrainer(null)} className="px-5 py-2.5 bg-white border border-[#CCFBF1] text-[#475569] font-medium rounded-xl hover:bg-[#F1F5F9] transition-colors">
                Close
              </button>
              <Link to={`/member/book-session?trainerId=${selectedTrainer._id}`} className="px-5 py-2.5 bg-[#16A34A] text-white font-semibold rounded-xl hover:bg-[#15803D] transition-colors flex items-center gap-2 shadow-lg shadow-[#16A34A]/20">
                <Calendar size={18} />
                Book Session Now
              </Link>
            </div>
