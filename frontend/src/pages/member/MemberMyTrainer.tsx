      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-3xl p-6 border border-[#E8E5DA] shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-[#164A4A] font-medium mt-1 mb-4">{trainer.specialization}</p>
            
            <div className="flex items-center justify-center space-x-2 text-sm text-[#455250] mb-6">
              <Star className="text-yellow-400 fill-yellow-400" size={18} />
              <span className="font-bold text-[#202828]">{trainer.rating}</span>
              <span>({trainer.reviews} Reviews)</span>
            </div>

            <div className="w-full space-y-3">
              <button className="w-full py-3 bg-[#164A4A] text-white rounded-xl font-bold hover:bg-[#C6A77D] transition-all shadow-lg shadow-green-500/20 flex items-center justify-center space-x-2">
                <MessageCircle size={20} />
                <span>Message Trainer</span>
              </button>
              <button className="w-full py-3 bg-white text-[#164A4A] border-2 border-[#164A4A] rounded-xl font-bold hover:bg-green-50 transition-all flex items-center justify-center space-x-2">
                <Calendar size={20} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
              <div className="bg-[#F2EFE8] p-4 rounded-2xl flex items-center space-x-4 border border-[#E8E5DA]">
                ...
                <div>
                  ...
                  <p className="font-bold text-[#202828]">{trainer.experience}</p>
                </div>
              </div>
              <div className="bg-[#F2EFE8] p-4 rounded-2xl flex items-center space-x-4 border border-[#E8E5DA]">
                ...
                <div>
                  <p className="text-sm text-[#687B78]">Usual Location</p>
                  <p className="font-bold text-[#202828]">{trainer.location}</p>
                </div>
              </div>
            </div>
