...
const SAAS_PLANS = [
  {
    ...
    features: [
      ...
      { text: '1 AI-generated Workout Plan', included: true },
    ...
    ctaStyle: 'bg-white border-2 border-[#CCFBF1] text-[#1E293B] hover:border-[#16A34A] hover:bg-[#F0FDFA]',
    trial: true,
    popular: false,
  },
  {
    ...
    borderColor: 'border-[#CCFBF1]',
    priceMonthly: 799,
    priceAnnual: 7190,         // ~₹599/mo billed annually (save ₹1,598)
    priceAnnualPerMonth: 599,  // shown as ₹599/mo billed annually
    savingsAnnual: 1598,
    ...
    features: [
      ...
      { text: 'Member Attendance Tracking', included: true },
    ...
    ctaStyle: 'bg-white border-2 border-slate-300 text-[#1E293B] hover:border-[#16A34A] hover:bg-[#F0FDFA]',
    trial: false,
    popular: false,
  },
  {
    ...
    priceMonthly: 1499,
    priceAnnual: 13490,        // ~₹1,124/mo billed annually (save ₹4,498 → show 2998)
    priceAnnualPerMonth: 1124, // shown as ₹1,124/mo billed annually
    savingsAnnual: 4498,
    limits: { members: 500, trainers: 15, staff: 5, branches: 2 },
    features: [
      { text: 'Everything in Silver', included: true },
      { text: 'Member Management (up to 500)', included: true },
      ...
      { text: '1-on-1 Online Coaching', included: false },
    ...
    ctaText: 'Choose Gold',
    ctaStyle: 'bg-[#16A34A] text-white hover:bg-[#15803D] shadow-lg shadow-green-200',
    trial: false,
    popular: true,
  },
  {
    ...
    priceMonthly: 2499,
    priceAnnual: 22490,        // ~₹1,874/mo billed annually (save ₹7,498 → show 4998)
    priceAnnualPerMonth: 1874, // shown as ₹1,874/mo billed annually
    savingsAnnual: 7498,
    limits: { members: -1, trainers: -1, staff: -1, branches: 5 },
    features: [
      { text: 'Everything in Gold', included: true },
      { text: 'Unlimited Members & Trainers', included: true },
...
const LimitBadge = ({ label, value }: { label: string; value: number }) => (
  <div className="flex items-center justify-between text-xs py-1">
    ...
    <span className="font-bold text-[#1E293B]">{value === -1 ? '∞ Unlimited' : `Up to ${value}`}</span>
  </div>
);
...
const SubscriptionPlansPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F0FDFA] text-[#1E293B] py-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-14">
          <div className="flex items-center justify-center space-x-2 mb-8">
            ...
            <span className="text-2xl font-bold text-[#16A34A]">AI GYM</span>
          </div>
          <div className="inline-flex items-center space-x-2 bg-[#16A34A]/10 border border-[#16A34A]/30 rounded-full px-4 py-1.5 text-[#16A34A] text-xs font-bold uppercase tracking-widest mb-6">
            <Sparkles size={12} />
            <span>SaaS Platform Subscriptions</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight text-[#1E293B]">
            Power Your Gym with <span className="text-[#16A34A]">AI GYM</span>
          </h1>
          <p className="text-[#475569] text-lg mb-3 max-w-2xl mx-auto">
            Choose a plan that fits your gym's size and ambitions. All plans include our core AI management platform.
          </p>
          <p className="text-xs text-[#475569] mb-10">These are SaaS subscriptions for gym owners, not individual member plans.</p>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <span className={`text-sm font-semibold transition-colors ${!isAnnual ? 'text-[#1E293B]' : 'text-[#94A3B8]'}`}>
              Monthly
            </span>
            ...
            <div className="flex items-center gap-2">
              ...
              {isAnnual ? (
                ...
                <span className="inline-flex items-center gap-1 bg-[#F0FDFA] text-[#0D9488] border border-[#CCFBF1] text-xs px-3 py-1 rounded-full font-medium">
        ...
        {paymentStep && selectedPlan && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white border border-[#CCFBF1] rounded-2xl p-8 max-w-md w-full shadow-2xl">
              ...
              <h2 className="text-2xl font-bold text-[#1E293B] mb-1">Complete Your Purchase</h2>
              <p className="text-[#475569] text-sm mb-6">
                You're subscribing to the <span className="text-[#1E293B] font-bold">{selectedPlan.name} Plan</span>
                {' '}on <span className="font-bold text-[#16A34A]">{isAnnual ? 'Annual' : 'Monthly'}</span> billing
              </p>

              <div className="bg-[#F8FAFC] border border-[#CCFBF1] rounded-xl p-4 mb-6 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-[#475569]">Plan</span>
                  <span className="text-[#1E293B] font-semibold">{selectedPlan.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#475569]">Billing Cycle</span>
                  <span className={`font-bold ${isAnnual ? 'text-[#16A34A]' : 'text-[#1E293B]'}`}>
                    {isAnnual ? 'Annual (2 months free!)' : 'Monthly'}
                  </span>
                </div>
                {isAnnual && (
                  <div className="flex justify-between text-sm">
                    <span className="text-[#475569]">Per Month</span>
                    <span className="text-[#1E293B] font-semibold">₹{selectedPlan.priceAnnualPerMonth?.toLocaleString('en-IN')}/mo</span>
                  </div>
                )}
                <div className="flex justify-between text-sm border-t border-[#CCFBF1] pt-3">
                  <span className="text-[#475569] font-bold">Total Due Today</span>
                  <span className="text-[#16A34A] font-black text-lg">
                    {getPaymentTotal(selectedPlan).amount}
                    <span className="text-sm font-medium text-[#475569] ml-1">{getPaymentTotal(selectedPlan).period}</span>
                  </span>
                </div>
              ...
              <div className="space-y-3 mb-6">
                <input type="text" placeholder="Card Number" className="w-full bg-white border border-[#CCFBF1] text-[#1E293B] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#16A34A]" maxLength={19} />
                <div className="grid grid-cols-2 gap-3">
                  <input type="text" placeholder="MM / YY" className="bg-white border border-[#CCFBF1] text-[#1E293B] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#16A34A]" maxLength={7} />
                  <input type="text" placeholder="CVV" className="bg-white border border-[#CCFBF1] text-[#1E293B] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#16A34A]" maxLength={3} />
                </div>
                <input type="text" placeholder="Cardholder Name" className="w-full bg-white border border-[#CCFBF1] text-[#1E293B] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#16A34A]" />
              </div>

              <div className="flex space-x-3">
                <button
                  onClick={() => setPaymentStep(null)}
                  className="flex-1 py-3 bg-white border border-[#CCFBF1] text-[#1E293B] rounded-xl font-bold hover:bg-[#F0FDFA] transition-colors text-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-[#16A34A] to-[#0D9488] text-white rounded-xl font-bold hover:from-[#15803D] hover:to-[#0F766E] transition-all disabled:opacity-60 text-sm flex items-center justify-center space-x-2 shadow-lg shadow-green-200"
                >
                  {loading ? (
              ...
              <p className="text-center text-xs text-[#475569] mt-4">🔒 Secured by 256-bit SSL encryption</p>
            </div>
          </div>
        )}
          ...
          {SAAS_PLANS.map((plan) => {
            ...
            return (
              ...
              >
                {/* Popular ribbon */}
                {isPopular && (
                  <div className="bg-[#16A34A] text-white text-[10px] font-extrabold text-center py-2 uppercase tracking-widest">
                    ⭐ Most Popular
                  </div>
                )}
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="mb-5">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="font-bold text-[#1E293B] text-base">{plan.name}</h3>
                          <p className="text-[#475569] text-xs">{plan.tagline}</p>
                        </div>
                      </div>
                      {plan.badge && (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#F0FDFA] border border-[#CCFBF1] text-[#0D9488]">
                          {plan.badge}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-4">
                      <div className="flex items-baseline gap-1">
                        <span className={`text-4xl font-black tracking-tight ${isPopular ? 'text-[#16A34A]' : plan.key === 'PREMIUM' ? 'text-purple-600' : 'text-[#1E293B]'}`}>
                          {priceDisplay.main}
                        </span>
                        ...
                        <div className="mt-1 min-h-[1.5rem]">
                          {isAnnual ? (
                            <div className="space-y-0.5">
                              <p className="text-[#475569] text-xs">{priceDisplay.sub}</p>
                  ...
                  {/* Limits */}
                  <div className="bg-[#F8FAFC] rounded-xl p-3 mb-5 border border-[#CCFBF1]">
                    <p className="text-[10px] uppercase tracking-widest text-[#475569] font-bold mb-2">Platform Limits</p>
                  ...
                  <ul className="space-y-2 mb-4 flex-1">
                    {plan.features.map((f, i) => (
                      <li key={i} className={`flex items-start space-x-2 text-xs ${f.included ? 'text-[#1E293B]' : 'text-[#CBD5E1]'}`}>
                        {f.included ? (
                          <CheckCircle size={14} className={`shrink-0 mt-0.5 ${isPopular ? 'text-[#16A34A]' : plan.key === 'PREMIUM' ? 'text-purple-500' : 'text-[#0D9488]'}`} />
                        ) : (
                          <XCircle size={14} className="shrink-0 mt-0.5 text-[#E2E8F0]" />
                        )}
        ...
        {/* Bottom note */}
        <div className="bg-white border border-[#CCFBF1] rounded-2xl p-6 mb-8 text-center">
          <div className="flex items-center justify-center space-x-2 mb-3">
            <Lock size={18} className="text-[#16A34A]" />
            <h3 className="text-[#1E293B] font-bold">How Access Control Works</h3>
          </div>
        ...
        <div className="text-center">
          <button onClick={logout} className="text-sm text-[#475569] hover:text-[#16A34A] transition-colors flex items-center space-x-2 mx-auto">
