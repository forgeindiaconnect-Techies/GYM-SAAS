  ...
  return (
    <div className="min-h-screen bg-[#F1F5F3] text-[#202828] selection:bg-[#164A4A] selection:text-black">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <span className="text-[#164A4A] text-sm font-bold uppercase tracking-wider mb-4 block">Knowledge Base</span>
        <h1 className="text-4xl md:text-5xl font-bold mb-12">The AI GYM Blog</h1>

        <div className="grid md:grid-cols-3 gap-8">
          {posts.map((post, i) => (
            <div key={i} className="bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl overflow-hidden group hover:border-[#164A4A]/50 transition-colors cursor-pointer flex flex-col">
              <div className="h-48 overflow-hidden">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center justify-between text-xs text-[#455250] mb-3">
                  <span className="bg-[#E8E5DA] px-2 py-1 rounded text-[#202828]">{post.category}</span>
                  <span>{post.date}</span>
                </div>
                <h3 className="text-xl font-bold mb-4">{post.title}</h3>
                <div className="mt-auto flex items-center space-x-2 text-[#164A4A] text-sm font-semibold group-hover:underline">
