  ...
  return (
    <div className="min-h-screen bg-[#F0FDFA] text-[#1E293B] selection:bg-[#16A34A] selection:text-black">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <span className="text-[#16A34A] text-sm font-bold uppercase tracking-wider mb-4 block">Join Our Team</span>
        ...
        <div className="space-y-4">
          {jobs.map((job, idx) => (
            <div key={idx} className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#CCFBF1] hover:border-[#16A34A]/50 transition-colors group flex flex-col md:flex-row md:items-center justify-between cursor-pointer">
              <div>
                <h3 className="text-xl font-bold text-[#1E293B] mb-2">{job.title}</h3>
                <div className="flex flex-wrap items-center space-x-4 text-sm text-[#475569]">
              ...
              <button className="mt-4 md:mt-0 px-5 py-2.5 bg-[#E2E8F0] text-[#1E293B] font-semibold rounded-lg group-hover:bg-[#16A34A] group-hover:text-black transition-colors">
                Apply Now
              </button>
            </div>
          ))}
