  ...
  useEffect(() => {
    try {
      ...
      if (dbMembers && Array.isArray(dbMembers) && dbMembers.length > 0) {
        // Map members from mockDb into client format
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full bg-[#FFFFFF] border border-[#D3DFDA] rounded-xl pl-10 pr-10 py-2 text-sm focus:outline-none focus:border-[#164A4A] transition-colors"
          />
          {searchTerm && (
            <button
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredClients.map((client) => (
            <div
              ...
              className="bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl p-5 hover:border-[#164A4A]/30 transition-colors relative group"
            >
              <button className="absolute top-4 right-4 text-[#455250] hover:text-[#202828]">
                <MoreVertical size={18} />
              </button>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-[#E8E5DA] rounded-full flex items-center justify-center font-bold text-lg text-[#202828]">
                  {client.name ? client.name[0].toUpperCase() : 'U'}
                </div>
                <div>
                  <h3 className="font-bold text-lg text-[#202828]">{client.name}</h3>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      client.status === 'Active'
                        ? 'bg-green-500/10 text-[#164A4A]'
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2 bg-[#164A4A]/10 hover:bg-[#164A4A]/20 text-[#164A4A] rounded-xl text-sm font-bold transition-colors"
                >
                  <Activity size={16} /> Progress
                </button>
                <button
                  onClick={() => navigate('/trainer/messages')}
                  className="flex-1 flex items-center justify-center gap-2 py-2 bg-[#E8E5DA] hover:bg-[#CBD5E1] text-[#202828] rounded-xl text-sm font-bold transition-colors"
                >
