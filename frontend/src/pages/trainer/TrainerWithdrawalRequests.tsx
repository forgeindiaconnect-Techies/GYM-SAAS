    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#727975] text-sm mt-1">Track your pending payout requests</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#E8E5DA] rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-[#727975] font-semibold">No pending requests</p>
            <p className="text-[#A8ADA9] text-sm mt-1">You don't have any pending withdrawal requests</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-[#F1F5F9]">
                {requests.map((r, idx) => (
                  <tr key={idx} className="hover:bg-[#F2EFE8] transition-colors">
                    ...
                    <td className="px-5 py-4 text-right font-bold text-[#202522]">₹{r.amount?.toLocaleString('en-IN')}</td>
