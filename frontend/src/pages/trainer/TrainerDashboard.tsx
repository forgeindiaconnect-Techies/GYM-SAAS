...
const TrainerDashboard = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Welcome back, <span className="text-[#16A34A]">{user?.firstName || 'Trainer'}!</span></h1>
        <p className="text-[#475569]">Here's your coaching overview for today.</p>
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-5 hover:-translate-y-1 transition-transform">
            ...
            <h3 className="text-[#475569] text-sm font-medium">{stat.label}</h3>
        ...
        <div className="md:col-span-2 bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl hover:border-[#16A34A]/50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-[#E2E8F0] flex items-center justify-center text-sm font-bold">
                    {session.client[0]}
                  </div>
                  <div>
                    <h4 className="font-bold">{session.client}</h4>
                    <p className="text-xs text-[#475569]">{session.type}</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-[#16A34A]">{session.time}</div>
                  <div className="text-xs text-[#475569]">{session.duration}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6">
            {[
              ...
              <div key={i} className="flex gap-4 items-start">
                <div className="mt-1 text-[#16A34A]"><TrendingUp size={18} /></div>
                <div>
                  <h4 className="font-bold text-sm">{prog.name}</h4>
                  <p className="text-xs text-[#475569] mt-1"><span className="text-[#1E293B]">Goal:</span> {prog.goal}</p>
