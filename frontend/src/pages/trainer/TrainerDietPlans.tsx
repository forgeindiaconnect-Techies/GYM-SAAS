    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[#4A514D]">Manage meal plans and nutrition guides</p>
        </div>
        <button 
        ...
        {plans.map((plan, i) => (
          <div key={i} className="bg-[#FFFFFF] border border-[#DCD9CD] rounded-2xl p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs px-2 py-1 border border-[#DCD9CD] rounded-full text-[#4A514D]">{plan.type}</span>
            </div>
            
            <h3 className="font-bold text-xl mb-4">{plan.name}</h3>
            
            <div className="flex items-end gap-2 mb-6">
              <span className="text-3xl font-bold text-[#34483F]">{plan.cals}</span>
              <span className="text-sm text-[#4A514D] mb-1">kcal/day</span>
            </div>

            <div className="grid grid-cols-3 gap-2 border-t border-[#DCD9CD] pt-4">
               <div className="text-center p-2 bg-[#FFFFFF] rounded-lg border border-[#DCD9CD]">
                 <Leaf size={14} className="mx-auto text-green-500 mb-1" />
                 <div className="font-bold text-sm">{plan.p}</div>
                 <div className="text-[10px] text-[#4A514D]">PRO</div>
               </div>
               <div className="text-center p-2 bg-[#FFFFFF] rounded-lg border border-[#DCD9CD]">
                 <Flame size={14} className="mx-auto text-orange-500 mb-1" />
                 <div className="font-bold text-sm">{plan.c}</div>
                 <div className="text-[10px] text-[#4A514D]">CARB</div>
               </div>
               <div className="text-center p-2 bg-[#FFFFFF] rounded-lg border border-[#DCD9CD]">
                 <Droplets size={14} className="mx-auto text-yellow-500 mb-1" />
                 <div className="font-bold text-sm">{plan.f}</div>
                 <div className="text-[10px] text-[#4A514D]">FAT</div>
               </div>
            </div>
            
            <button className="w-full mt-6 py-2 border border-[#DCD9CD] hover:bg-[#E8E5DA] text-[#202522] rounded-xl text-sm font-medium transition-colors">
              View Details
            </button>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#FFFFFF] rounded-2xl max-w-md w-full shadow-2xl border border-[#DCD9CD] overflow-hidden">
            <div className="p-6 border-b border-[#DCD9CD] flex justify-between items-center bg-[#F2EFE8]">
              <h2 className="text-xl font-bold text-[#202522]">Create Diet Plan</h2>
              <button 
            ...
            <form onSubmit={handleCreatePlan} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#4A514D] mb-2">Daily Calories *</label>
                  <input 
                    type="number"
                    value={form.cals}
                ...
                <div>
                  <label className="block text-sm font-medium text-[#4A514D] mb-2">Plan Type *</label>
                  <select 
                    value={form.type}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#4A514D] mb-2">Protein *</label>
                  <input 
                    type="text"
                    value={form.p}
                ...
                <div>
                  <label className="block text-sm font-medium text-[#4A514D] mb-2">Carbs *</label>
                  <input 
                    type="text"
                    value={form.c}
                ...
                <div>
                  <label className="block text-sm font-medium text-[#4A514D] mb-2">Fat *</label>
                  <input 
                    type="text"
                    value={form.f}
