...
const TrainerNotifications = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-end border-b border-[#DCD9CD] pb-4">
        <div>
          <h1 className="text-3xl font-bold">Notifications</h1>
          <p className="text-[#4A514D]">Alerts, messages and updates</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#4A514D]">
            {notifications.filter(n => !n.isRead).length} unread
          </span>
          <button
            onClick={markAllAsRead}
            disabled={notifications.every(n => n.isRead)}
            className="inline-flex items-center gap-1.5 text-sm text-[#34483F] hover:underline disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <CheckCheck size={15} />
            Mark all as read
          </button>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#DCD9CD] rounded-2xl py-16 text-center">
          <Bell size={40} className="mx-auto text-[#CBD5E1] mb-3" />
          <p className="text-[#727975] font-semibold">No notifications yet</p>
          <p className="text-[#A8ADA9] text-sm mt-1">Alerts, messages and updates will appear here.</p>
        </div>
      ) : (
          ...
          {notifications.map(notif => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border transition-colors ${!notif.isRead ? 'bg-[#FFFFFF] border-[#DCD9CD]' : 'bg-transparent border-transparent'} flex gap-4 hover:border-[#34483F]/40`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1 gap-3">
                    <h4 className={`font-bold truncate ${!notif.isRead ? 'text-[#202522]' : 'text-[#4A514D]'}`}>{notif.title}</h4>
                  ...
                  <p className="text-sm text-[#4A514D] line-clamp-2">{notif.message}</p>
                </div>
                {!notif.isRead && (
                  <span className="w-2.5 h-2.5 rounded-full bg-[#34483F] shrink-0 self-center" />
                )}
              </button>
            );
          })}
