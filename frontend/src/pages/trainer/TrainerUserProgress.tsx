      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="bg-[#FFFFFF] border border-[#D3DFDA] rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-4 border-b border-[#D3DFDA] pb-4">
            ...
            <div className="flex justify-between items-end mb-4">
              <div>
                <p className="text-sm text-[#455250] mb-1">{prog.metric}</p>
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold">{prog.old}</span>
                  <span className="text-[#455250]">→</span>
                  <span className={`text-xl font-bold ${prog.trend === 'up' ? 'text-[#164A4A]' : 'text-green-500'}`}>{prog.new}</span>
                </div>
              </div>
              <div className={`p-2 rounded-lg ${prog.trend === 'up' ? 'bg-[#164A4A]/10 text-[#164A4A]' : 'bg-green-500/10 text-green-500'}`}>
                <TrendingUp size={20} className={prog.trend === 'down' ? 'rotate-180' : ''} />
              </div>
            </div>
            <p className="text-sm text-[#455250] flex items-center gap-2">
...
export default TrainerUserProgress;