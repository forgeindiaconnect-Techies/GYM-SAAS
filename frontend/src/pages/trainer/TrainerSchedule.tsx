...
const TrainerSchedule = () => {
  ...
  const handleAddBlock = (e: React.FormEvent) => {
    ...
    setForm({ time: '09:00 AM', type: 'Personal Training', client: '', duration: '60 min', color: 'bg-[#16A34A]' });
  };
      ...
      <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-[#CCFBF1] flex justify-between items-center bg-[#FFFFFF]">
          ...
          <div className="flex gap-2">
            <button className="p-2 border border-[#CCFBF1] rounded-lg hover:bg-[#E2E8F0] transition-colors"><ChevronLeft size={16} /></button>
            <button className="p-2 border border-[#CCFBF1] rounded-lg hover:bg-[#E2E8F0] transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
            ...
            {events.map((event, i) => (
              <div key={i} className="relative pl-6">
                ...
                <div className="bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl p-4 flex justify-between items-center hover:border-[#16A34A]/30 transition-colors cursor-pointer">
                  ...
                  <button className="px-4 py-2 border border-[#CCFBF1] rounded-lg text-sm hover:bg-[#E2E8F0] transition-colors">Details</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#FFFFFF] rounded-2xl max-w-md w-full shadow-2xl border border-[#CCFBF1] overflow-hidden">
            <div className="p-6 border-b border-[#CCFBF1] flex justify-between items-center bg-[#F8FAFC]">
              <h2 className="text-xl font-bold text-[#1E293B]">Add Schedule Block</h2>
              <button 
            ...
            <form onSubmit={handleAddBlock} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-medium text-[#475569] mb-2">Event Type *</label>
                <select 
                  value={form.type}
              ...
              <div>
                <label className="block text-sm font-medium text-[#475569] mb-2">Client / Group Name *</label>
                <input 
                  type="text"
                  value={form.client}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#475569] mb-2">Time *</label>
                  <input 
                    type="text"
                    value={form.time}
                ...
                <div>
                  <label className="block text-sm font-medium text-[#475569] mb-2">Duration *</label>
                  <select 
                    value={form.duration}
              ...
              <div>
                <label className="block text-sm font-medium text-[#475569] mb-2">Color Label</label>
                <div className="flex gap-3">
                  {[
                    { value: 'bg-[#16A34A]', label: 'Green' },
