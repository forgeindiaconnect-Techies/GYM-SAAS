    ...
    <>
      ...
      <div className="p-5 border-b border-[#D3DFDA]">
        <Link to="/" className="flex items-center space-x-2" onClick={() => setSidebarOpen(false)}>
          <div className="w-9 h-9 bg-gradient-to-br from-[#6fa3a0] to-[#D2B48C] rounded-xl flex items-center justify-center shadow-lg shadow-teal-200">
            <ShieldCheck className="text-[#202828]" size={20} />
          </div>
          <div className="flex flex-col leading-tight">
            <span className="text-sm font-bold tracking-tight text-[#6fa3a0]">SYSTEM</span>
            <span className="text-sm font-bold tracking-tight text-[#164A4A]">ADMIN</span>
          </div>
        </Link>
        <div className="mt-4 flex items-center space-x-3">
          <div className="w-10 h-10 bg-gradient-to-br from-[#6fa3a0] to-[#D2B48C] rounded-full flex items-center justify-center text-[#202828] font-bold text-base shadow">
            {user?.firstName?.[0] || 'S'}
          </div>
          <div className="flex-1 overflow-hidden">
            <p className="font-semibold text-sm text-[#202828] truncate">{user?.firstName || 'Super'} {user?.lastName || 'Admin'}</p>
            <p className="text-xs text-[#6fa3a0] font-medium truncate">Root Access</p>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative min-w-0">
        ...
        <header className="h-16 border-b border-[#D3DFDA] flex items-center px-4 md:px-8 justify-between bg-white/90 backdrop-blur-md z-10 sticky top-0 shrink-0 shadow-sm">
          <div className="flex items-center space-x-3">
            <button
              className="lg:hidden text-[#455250] hover:text-[#6fa3a0] transition-colors p-1 rounded-lg hover:bg-[#F1F5F3]"
              onClick={() => setSidebarOpen(true)}
              aria-label="Open menu"
            >
              <Menu size={22} />
            </button>
            {currentNav && <currentNav.icon size={20} className="text-[#6fa3a0] hidden sm:block" />}
          ...
          <div className="flex items-center space-x-3">
            <Link to="/super-admin/profile" className="flex items-center space-x-2 hover:opacity-80 transition-opacity">
              <div className="text-right hidden md:block">
                <p className="text-sm font-semibold text-[#202828] leading-none mb-0.5">{user?.firstName || 'Super'} {user?.lastName || 'Admin'}</p>
                <p className="text-xs text-[#6fa3a0] font-medium leading-none">Root Access</p>
              </div>
              <div className="w-9 h-9 bg-gradient-to-br from-[#6fa3a0] to-[#D2B48C] rounded-full flex items-center justify-center text-[#202828] font-bold text-sm shadow">
                {user?.firstName?.[0] || 'S'}
              </div>
            </Link>
          </div>
