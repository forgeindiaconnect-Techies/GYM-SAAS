...
export const Step8Review = ({ data, setStep }: any) => {
  const SectionHeader = ({ title, step }: { title: string, step: number }) => (
    <div className="flex justify-between items-center border-b border-[#CCFBF1] pb-2 mb-4">
      <h3 className="text-lg font-bold text-[#1E293B]">{title}</h3>
      <button onClick={() => setStep(step)} className="text-xs font-bold text-[#16A34A] hover:underline px-3 py-1 bg-[#16A34A]/10 rounded-lg transition-colors">Edit</button>
    </div>
  );
  ...
  const DataRow = ({ label, value }: { label: string, value: any }) => (
    <div className="grid grid-cols-3 py-2 border-b border-[#FFFFFF]">
      ...
      <span className="col-span-2 text-[#1E293B] text-sm font-medium">{value || <span className="text-gray-600 italic">Not provided</span>}</span>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <section className="bg-[#FFFFFF] p-5 rounded-2xl border border-[#CCFBF1]">
          ...
          <section className="bg-[#FFFFFF] p-5 rounded-2xl border border-[#CCFBF1]">
            ...
            <div className="space-y-3 mt-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between bg-[#FFFFFF] p-3 rounded-lg border border-[#CCFBF1]">
                  <div className="flex items-center space-x-3 overflow-hidden">
                    <FileText size={16} className={doc.file ? 'text-green-500' : 'text-[#475569]'} />
                    <span className="text-sm font-semibold text-[#1E293B] truncate">{doc.label}</span>
                  </div>
                  {doc.file ? (
                    <span className="text-xs bg-green-500/20 text-green-500 px-2 py-1 rounded-md font-bold">Uploaded</span>
                  ) : (
                    <span className="text-xs bg-[#0D9488]/20 text-[#0D9488] px-2 py-1 rounded-md font-bold">Missing</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          {data.operatingHours?.map((h: any) => (
            <div key={h.dayOfWeek} className="bg-[#FFFFFF] p-3 rounded-xl">
              <span className="text-xs text-[#475569] block">{h.dayOfWeek}</span>
              <span className={`text-sm font-bold ${h.isOpen ? 'text-[#1E293B]' : 'text-[#0D9488]'}`}>
      ...
      <section className="bg-[#FFFFFF] p-5 rounded-2xl border border-[#CCFBF1]">
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-4">
          {data.plans?.map((p: any) => (
            <div key={p.id} className="bg-[#FFFFFF] p-4 rounded-xl border border-[#CCFBF1]">
              <div className="flex justify-between items-start">
                <span className="font-bold text-[#1E293B]">{p.planName}</span>
                <span className="text-xs bg-green-500/20 text-green-500 px-2 rounded-full">{p.status}</span>
              </div>
              <p className="text-[#EF4444] font-bold mt-2 flex items-center">₹{p.finalPrice} <span className="text-xs text-[#475569] font-normal ml-1">/ {p.duration} {p.durationUnit}</span></p>
            </div>
          ))}
          {!data.plans?.length && (
            <div className="col-span-3 text-center py-4 text-[#475569] text-sm">No plans configured.</div>
          )}
        </div>
      </section>
    </div>
