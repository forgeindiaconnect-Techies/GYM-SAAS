  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-[#CCFBF1] pb-2 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold text-[#1E293B]">Promotional Offers</h2>
          <p className="text-sm text-[#475569]">Create special deals and discounts for customers.</p>
        </div>
        {!showAddOffer && (
      ...
      {showAddOffer && (
        <div className="bg-[#FFFFFF] border border-[#22C55E]/30 rounded-xl p-5 space-y-4">
          <h3 className="font-medium text-[#1E293B] text-sm">New Offer Details</h3>
          
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#475569] mb-1">Offer Name *</label>
            ...
            <div>
              <label className="block text-xs text-[#475569] mb-1">Offer Type *</label>
          ...
          <div>
            <label className="block text-xs text-[#475569] mb-1">Description</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-[#475569] mb-1">Original Price</label>
            ...
            <div>
              <label className="block text-xs text-[#475569] mb-1">Offer Price</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#475569] mb-1">Valid From *</label>
            ...
            <div>
              <label className="block text-xs text-[#475569] mb-1">Valid Until *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#475569] mb-1">Applicable Plan</label>
          ...
          <div>
            <label className="block text-xs text-[#475569] mb-1">Terms & Conditions *</label>
          ...
          <div className="flex justify-end space-x-3 pt-2">
            <button type="button" onClick={() => setShowAddOffer(false)} className="px-4 py-2 text-sm text-[#475569] hover:text-[#16A34A] transition-colors">Cancel</button>
      ...
      {offers.length === 0 && !showAddOffer ? (
        <div className="text-center py-8 bg-[#FFFFFF] border border-[#CCFBF1] border-dashed rounded-xl">
          <p className="text-[#475569] text-sm mb-3">No promotional offers active.</p>
          <button type="button" onClick={() => setShowAddOffer(true)} className="px-4 py-2 bg-[#FFFFFF] text-[#1E293B] text-sm border border-[#CCFBF1] rounded-lg hover:border-[#22C55E]/50 transition-colors">
            Create an Offer
          </button>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 gap-4">
          {offers.map((offer: any, i: number) => (
            <div key={i} className="bg-[#FFFFFF] border border-[#CCFBF1] p-5 rounded-xl relative group">
              <button type="button" onClick={() => removeOffer(i)} className="absolute top-3 right-3 text-[#475569] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                <Trash2 size={16} />
              </button>
              <div className="flex items-center space-x-2 mb-2">
                <Tag size={16} className="text-[#22C55E]" />
                <h3 className="text-sm font-bold text-[#1E293B]">{offer.name}</h3>
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${offer.status === 'Active' ? 'bg-[#22C55E]/20 text-[#22C55E]' : 'bg-[#555]/20 text-[#475569]'}`}>
                  {offer.status}
                </span>
              </div>
              <p className="text-xs text-[#475569] mb-3">{offer.type}</p>
              
              <div className="grid grid-cols-2 gap-2 text-xs mb-3 bg-[#FFFFFF] p-2 rounded-lg">
                <div>
                  <span className="text-[#475569] block">Valid From</span>
                  <span className="text-[#1E293B]">{offer.validFrom}</span>
                </div>
                <div>
                  <span className="text-[#475569] block">Valid Until</span>
                  <span className="text-[#1E293B]">{offer.validUntil}</span>
                </div>
              </div>
              <p className="text-[10px] text-[#475569]">T&C: {offer.terms}</p>
            </div>
          ))}
        </div>
      )}
