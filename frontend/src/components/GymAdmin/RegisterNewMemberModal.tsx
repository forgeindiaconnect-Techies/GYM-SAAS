    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-[#FFFFFF] rounded-2xl max-w-3xl w-full shadow-2xl border border-[#DCD9CD] overflow-hidden my-8 shrink-0">
        ...
        <div className="p-6 border-b border-[#DCD9CD] flex justify-between items-center bg-[#F2EFE8]">
          <div>
            <h2 className="text-xl font-bold text-[#202522]">Register New Customer</h2>
            <p className="text-sm text-[#4A514D] mt-1">Step {step} of 5</p>
          </div>
          <button onClick={onClose} type="button" className="text-[#4A514D] hover:text-[#202522] transition-colors p-2 rounded-lg hover:bg-gray-100">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="w-full bg-gray-200 h-1.5">
          <div className="bg-[#34483F] h-1.5 transition-all duration-300" style={{ width: `${(step / 5) * 100}%` }}></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in">
              <div className="flex items-center space-x-3 text-[#34483F] mb-2 border-b border-[#DCD9CD] pb-2">
                <User size={20} />
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]">
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <p className="text-sm text-[#4A514D] mb-4">Provide basic metrics to initialize their AI Assessment.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-[#4A514D] mb-1">Height (cm)</label>
                  <input type="number" value={formData.height} onChange={e => setFormData({...formData, height: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]" placeholder="e.g. 175" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#4A514D] mb-1">Current Weight (kg)</label>
                  <input type="number" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]" placeholder="e.g. 70" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#4A514D] mb-1">Fitness Goal</label>
                  <select value={formData.fitnessGoal} onChange={e => setFormData({...formData, fitnessGoal: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]">
                ...
                <div>
                  <label className="block text-sm font-bold text-[#4A514D] mb-1">Fitness Level</label>
                  <select value={formData.fitnessLevel} onChange={e => setFormData({...formData, fitnessLevel: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]">
            ...
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input value={formData.emergencyName} onChange={e => setFormData({...formData, emergencyName: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]" placeholder="Emergency Contact Name" />
                </div>
                <div>
                  ...
                  }} minLength={10} maxLength={10} pattern="\d{10}" title="Please enter exactly 10 digits" className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF] invalid:[&:not(:placeholder-shown):not(:focus)]:border-red-500" placeholder="Phone Number" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#4A514D] mb-1">Relationship</label>
                  <input value={formData.emergencyRelation} onChange={e => setFormData({...formData, emergencyRelation: e.target.value})} className="w-full bg-[#F2EFE8] border border-[#DCD9CD] rounded-xl px-4 py-2.5 outline-none focus:border-[#34483F] focus:bg-[#FFFFFF]" placeholder="e.g. Parent" />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="bg-[#F5F3EE] p-6 rounded-xl border border-[#DCD9CD]">
                <p className="text-[#202522] font-medium mb-4">By completing this registration, a secure member account will be automatically generated and linked to this gym.</p>
                <div className="flex flex-col space-y-4">
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    <span className="text-[#202522] font-medium">Create Member Login Account Now</span>
                  </label>
                  <label className="flex items-center space-x-3 cursor-pointer">
                    ...
                    <span className="text-[#202522] font-medium">Send Welcome Email & Setup Instructions</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          {/* Controls */}
          <div className="flex justify-between items-center pt-6 border-t border-[#DCD9CD]">
            <button 
              type="button" 
              onClick={step === 1 ? onClose : prevStep} 
              className="px-6 py-2.5 text-[#4A514D] font-bold hover:bg-gray-100 rounded-xl transition-colors flex items-center"
            >
