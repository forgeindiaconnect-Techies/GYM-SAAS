    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#CCFBF1]">
        <div className="flex items-center space-x-3">
          <UserCheck className="text-[#16A34A]" size={24} />
      ...
      {data.members.length === 0 && !isAdding && (
        <div className="text-center py-12 border-2 border-dashed border-[#CCFBF1] rounded-2xl">
          <UserCheck className="text-[#475569] mx-auto mb-3" size={40} />
          <p className="text-[#475569]">No initial members added.</p>
          <button onClick={() => setIsAdding(true)} className="mt-4 text-[#16A34A] font-semibold hover:underline">Add a member (Optional)</button>
        </div>
      )}
      ...
      {data.members.length > 0 && !isAdding && (
        <div className="grid md:grid-cols-2 gap-4">
          {data.members.map((m: any) => (
            <div key={m.id} className="bg-[#FFFFFF] p-4 rounded-xl border border-[#CCFBF1] flex justify-between items-start">
              <div>
                <h4 className="text-[#1E293B] font-bold">{m.name}</h4>
                <p className="text-sm text-[#16A34A] font-medium">{m.planName}</p>
                <p className="text-xs text-[#475569] mt-1">{m.phone} • Joined: {m.joiningDate}</p>
              </div>
              <button onClick={() => removeMember(m.id)} className="text-[#475569] hover:text-[#0D9488] transition-colors">
                <X size={18} />
              </button>
            </div>
          ))}
      ...
      {isAdding && (
        <div className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#CCFBF1]">
          ...
          {data.plans.length === 0 ? (
            <div className="p-4 bg-[#0D9488]/10 text-[#0D9488] rounded-xl text-sm">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-[#475569] mb-1">Email (Optional)</label>
              ...
              <div>
                <label className="block text-xs text-[#475569] mb-1">Select Plan *</label>
                <select value={form.planId} onChange={e => setForm({...form, planId: e.target.value})} className="w-full bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl px-3 py-2 text-sm text-[#1E293B] focus:border-[#16A34A] outline-none appearance-none">
              ...
              <div>
                <label className="block text-xs text-[#475569] mb-1">Joining Date *</label>
                <input type="date" value={form.joiningDate} onChange={e => setForm({...form, joiningDate: e.target.value})} className="w-full bg-[#FFFFFF] border border-[#CCFBF1] rounded-xl px-3 py-2 text-sm text-[#1E293B] focus:border-[#16A34A] outline-none [color-scheme:dark]" />
              </div>
            </div>
          )}
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <button onClick={handleAdd} disabled={data.plans.length === 0} className="px-6 py-2 bg-[#16A34A] text-white rounded-xl font-bold text-sm disabled:opacity-50">Save Member</button>
          </div>
        </div>
      )}
    </div>
